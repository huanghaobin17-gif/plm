    ...
    if (empty($baseUrl)) {
        if ('@' == $class_strut[0] || MODULE_NAME == $class_strut[0]) {
            ...
            $class   = substr($class, 7);
                ...
                ['Think', 'Org', 'Behavior', 'Com', 'Vendor']) || is_dir(LIB_PATH . $class_strut[0])) {
            // 系统类库包和第三方类库包
            $baseUrl = LIB_PATH;
        } else {
            // 加载其他模块的类库
...
{
    if (App\Service\UserInfo\UserInfo::getInstance()->judgeStart() && App\Service\UserInfo\UserInfo::getPlatform() == C('VUE_APP_APP')) {
        return cacheToken($name, $value);
    }
 ...
 *
 * @return mixed
 * @see \App\Service\UserInfo\Token
 *
 */
function cacheToken($key, $value = '')
{
    if ($key === '') {
        return App\Service\UserInfo\UserInfo::getInstance()->getAll();
    }
    if ($value === '') {
        return App\Service\UserInfo\UserInfo::getInstance()->get($key);
    } else {
        return App\Service\UserInfo\UserInfo::getInstance()->set($key, $value);
    }
}
