...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util'], function (exports) {
  ...
  Class.prototype.renderToolbar = function () {
    ...
    if (typeof options.defaultToolbar === 'object') {
      layui.each(options.defaultToolbar, function (i, item) {
        ...
        if (thisItem) {
          iconElem.push('<a title="' + thisItem.title + '" lay-event="' + thisItem.layEvent + '">' + thisItem.title
              + '</a>');
        }
      });
    }
  ...
  Class.prototype.renderTotal = function (data) {
    ...
    that.eachCols(function (i3, item3) {
      ...
      //td内容
      var td = ['<td data-key="' + options.index + '-' + item3.key + '" ' + function () {
        var attr = [];
        if (item3.align) attr.push('align="' + item3.align + '"'); //对齐方式
        if (item3.style) attr.push('style="' + item3.style + '"'); //自定义样式
        if (item3.minWidth) attr.push('data-minwidth="' + item3.minWidth + '"'); //单元格最小宽度
        ...
        , '<div class="layui-table-cell laytable-cell-' + function () { //返回对应的CSS类标识
          ...
          if (totalNums[field]>99.99&&totalNums[field]<100.01) {
             totalNums[field] = 100.00;
          }
          return item3.totalRow ? (parseFloat(totalNums[field]).toFixed(item3.totalNums!=undefined?item3.totalNums:2)) || text : text;
          //
        }() 
        , '</div></td>'].join('');
      tds.push(td);
    ...
    that.layTool.on('click', '*[lay-event]', function (e) {
      ...
      switch (events) {
        case 'LAYTABLE_COLS': //筛选列
          openPanel({
            list: function () {
              //if ($(options.elem).parents().hasClass('layui-fluid')) {
              //  var sessionUseridVal = $("input[name='sessionUserid']").val();
              //  var pageId = $(options.elem).parents('.layui-fluid').attr('id'),
              //      oldData = JSON.parse(localStorage.getItem([pageId + sessionUseridVal]));
              //}
              var lis = [];
              //var changeList = [];
              that.eachCols(function (i, item) {
                /*改源码部分 by liuwl*/
                //if (oldData) {
                //  if (oldData[0][i]['title'] == item.title) {
                //    item.hide = oldData[0][i]['hide'];
                //  }
                //}
                /**/
                if (item.field && item.type == 'normal') {
                  ...
                  //changeList.push({dataKey: item.key, dataParentkey: item.parentKey || ''});
                }
              });
              /*改源码部分 by liuwl*/
              //if (oldData) {
              //  layui.each(oldData[0], function (k, v) {
              //    if (v.hide == true) {
              //      that.elem.find('*[data-key="' + options.index + '-' + v.key + '"]').addClass(HIDE);
              //    }
              //    //重新适配尺寸
              //    that.resize();
              //  });
            ...
            , done: function () {
              form.on('checkbox(LAY_TABLE_TOOL_COLS)', function (obj) {
                ...
                //localStorage.setItem([pageId + sessionUseridVal], JSON.stringify(newEachData));
                //}
                /**/
              });
            }
