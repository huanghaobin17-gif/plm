        ...
        search: function(){
            (function (factory) {
                ...
                function adjustDropMenuPos($input, $dropdownMenu, options) {
                    ...
                    // 列表对齐方式
                    var dmcss = {};
                    var thisMenuWidth = parseInt(options['listStyle']['width'].substring(0,options['listStyle']['width'].length-2)),
                        windowsWidth = $(window).width(),
                        inputObj = $($parent[0]);
                    if (thisMenuWidth > windowsWidth - inputObj.offset().left - 35 && thisMenuWidth < inputObj.offset().left){
                        options.listAlign = 'right';
                    }else {
                        if (windowsWidth < 992){
                            if (inputObj.offset().left + inputObj.width() > thisMenuWidth){
                                options.listAlign = 'right';
                            }else {
                                options.listAlign = 'left';
                            }
                        }else {
                            options.listAlign = 'left';
                        }
                ...
                var defaultOptions = {
                    ...
                    listHoverStyle: 'background: #5FB878; color:#fff', // 提示框列表鼠标悬浮的样式
