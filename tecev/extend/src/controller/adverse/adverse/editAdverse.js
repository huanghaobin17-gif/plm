layui.define(function(exports){
    layui.use(['layer', 'form', 'upload','laydate','suggest','tipsType'], function(){
        ...
        upload.render({
            elem: '#choose' //绑定元素
            ,url: 'editAdverse?type=uploadFile' //上传接口
        ...
        $("#productName").bsSuggest(
            {
                url: admin_name+'/Adverse/editAdverse?type=getAllAssetsSearch',
...
//查看上传文件
    $(document).on('click', '.fileNameInput', function (){
        var suffix = $(".fileName").attr('data-type');
        var path = $(".fileName").attr('data-path');
        var name = $(".fileName").attr('data-name');
...
//删除文件按钮
    ...
    exports('controller/adverse/adverse/editAdverse', {});
});
