layui.define(function(exports){
    layui.use(['admin', 'layer', 'form', 'formSelects', 'laydate', 'table', 'suggest', 'tablePlug'], function () {
        ...
        //操作栏按钮
        table.on('tool(testReportData)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
            var rows = obj.data; //获得当前行数据
            var layEvent = obj.event; //获得 lay-event 对应的值（也可以是表头的 event 参数对应的值）
            var flag = 1;
            var url = $(this).attr('data-url');
            switch (layEvent){
                case 'testReport'://制定计划
                    top.layer.open({
                        type: 2,
                        title: '【'+rows.assets_name+'】上传设备测试运行报告',
                        area: ['780px', '100%'],
                        offset: 'r',//弹窗位置固定在右边
                        anim: 2, //动画风格
                        scrollbar:false,
                    ...
                    break;
                case 'showTest'://编辑主设备详情
                    top.layer.open({
                        type: 2,
                        title: '【'+rows.assets_name+'】查看设备测试运行报告',
                        area: ['780px', '100%'],
                        offset: 'r',//弹窗位置固定在右边
                        anim: 2, //动画风格
                        scrollbar:false,
        ...
        $('#testReset').on('click',function () {
            $('input[name="test_departid"]').val(0);
        });
        //科室搜索建议
        $("#testListDepartment").bsSuggest(
            returnDepartment()
        ).on('onSetSelectValue', function (e, keyword, data) {
            departid = data.departid;
            $('input[name="test_departid"]').val(departid);
        });
        /*
         /选择设备字典名称
         */
        $("#test_assets_name").bsSuggest(
            returnDicAssets(hospital_id)
        );

    });
    exports('purchases/purchaseCheck/testReportList', {});
});