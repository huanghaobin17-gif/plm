...
(function (layui, window, factory) {
	...
	var v = '4.0.0.0910',
		...
		CHECKBOX_YES = 'layui-icon layui-icon-ok',
		...
		FormSelects = function FormSelects(options) {
			...
			$.extend(this.config, options, {
				...
				btns: options.radio ? [quickBtns[1]] : [quickBtns[0], quickBtns[1]]
	...
	Select4.prototype.btns = function (id, btns, config) {
		...
		$.each(target, function (key, val) {
			...
			if (btns.length) {
				...
				var html = common.renderBtns(key, show, config && config.space ? config.space : '10px');
				dd.html(html);
			} else {
