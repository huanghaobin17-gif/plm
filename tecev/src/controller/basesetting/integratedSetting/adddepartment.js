layui.define(function(exports){
    layui.use(['layer', 'form'], function() {
        ...
        form.on('select(change_hospital)', function (data) {
            ...
            $.getJSON(admin_name+'/IntegratedSetting/addDepartment?action=getParentNum&hospital_id='+data.value,function(result){
                if (result.existNum == null){
                    $(".changeDepartNum").html('暂无');
                }else {
                    $(".changeDepartNum").html(result.existNum);
                }
                $("input[name='departnum']").val(result.maxNum)
            });
        });
    });
    exports('controller/basesetting/integratedSetting/adddepartment', {});
});
