layui.define(function(exports){
    layui.use(['form','upload'], function(){
        ...
        form.render();
        //监听提交
        form.on('submit(saveAssessReport)', function(data){
            var params = data.field;
            params.action = 'finalSave';
            submit($,params,'assessReport');
            return false;
        });
        //上传附件
        upload.render({
            elem: '#addReport'  //绑定元素
            , url: admin_name+'/PurchaseCheck/assessReport' //接口
            , accept: 'file'
            , exts: 'jpg|png|bmp|jpeg|doc|docx|pdf|zip' //格式 用|分隔
            , method: 'POST'
            , data: {action: 'uploadFile',train_id:$('input[name="train_id"]').val()}
            , choose: function (obj) {
    ...
    exports('controller/purchases/purchaseCheck/assessReport', {});
});
