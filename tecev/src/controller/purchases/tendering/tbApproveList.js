layui.define(function(exports){
    layui.use(['admin', 'layer', 'form', 'formSelects', 'laydate', 'table', 'suggest', 'tablePlug'], function () {
        ...
        //监听搜索
        form.on('submit(tpSearch)', function (data) {
            gloabOptions = data.field;
            //搜索时重新初始化全局搜索参数，方便修改删除用户时候刷表格面用
            //刷新表格时，默认回到第一页
            var table = layui.table;
            //table.set(gloabOptions);
            table.reload('tbApproveList', {
                url: tbApproveList
                , where: gloabOptions
                , page: {
                    curr: 1 //重新从第 1 页开始
        ...
        //操作栏按钮
        table.on('tool(tbApproveData)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
            ...
            switch (layEvent){
                case 'tbApprove'://制定标书
                    ...
                    break;
                case 'showTbApprove'://制定标书
                    top.layer.open({
                        type: 2,
                        title: '【'+rows.assets_name+'】查看评审',
                        area: ['700px', '100%'],
                        offset: 'r',//弹窗位置固定在右边
                        anim: 2, //动画风格
                        scrollbar:false,
        ...
        //列排序
        table.on('sort(tbApproveData)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
            table.reload('tbApproveList', {
                initSort: obj
                ,where: {
                    sort: obj.field //排序字段
        ...
        function getAssetsDic(id) {
            /*
             /选择设备名称
             */
            $("#tpAssetsName").bsSuggest(
                returnDicAssets(id)
            );
        }
    });
    exports('purchases/tendering/tbApproveList', {});
});
