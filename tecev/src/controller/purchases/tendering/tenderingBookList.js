layui.define(function(exports){
    layui.use(['layer', 'form', 'formSelects', 'laydate', 'table', 'suggest', 'tablePlug'], function () {
        ...
        //录入时间元素渲染
        laydate.render({
            elem: '#bookStartDate' //指定元素
        });
        laydate.render({
            elem: '#bookEndDate' //指定元素
        });
        ...
        var gloabOptions = {};
        table.render({
            elem: '#tenderingBookList'
            //,height: '600'
            , limits: [10, 20, 50, 100]
            , loading: true
            , limit: 10
            , url: tenderingBookList //数据接口
            , where: {
                sort: 'record_id',
                order: 'desc'
            } //如果无需传递额外参数，可不加该参数
            ...
            , cols: [[ //表头
                ...
                {field: 'apply_type_name', title: '申请方式', width: 120, align: 'center'},
                ...
                {field: 'market_price', title: '设备单价', width: 120, align: 'center'},
        ...
        //监听搜索
        form.on('submit(bookListSearch)', function (data) {
            gloabOptions = data.field;
            //搜索时重新初始化全局搜索参数，方便修改删除用户时候刷表格面用
            //刷新表格时，默认回到第一页
            var table = layui.table;
            //table.set(gloabOptions);
            table.reload('tenderingBookList', {
                url: tenderingBookList
                , where: gloabOptions
                , page: {
                    curr: 1 //重新从第 1 页开始
                }
        ...
        //操作栏按钮
        table.on('tool(tenderingBookData)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
            var rows = obj.data; //获得当前行数据
            var layEvent = obj.event; //获得 lay-event 对应的值（也可以是表头的 event 参数对应的值）
            var flag = 1;
            var url = $(this).attr('data-url');
            switch (layEvent){
                case 'addTenderingBook'://制定标书
                    top.layer.open({
                        type: 2,
                        title: '【'+rows.assets_name+'】制定标书',
                        area: ['700px', '100%'],
                        offset: 'r',//弹窗位置固定在右边
                        anim: 2, //动画风格
                        scrollbar:false,
        ...
        //列排序
        table.on('sort(tenderingBookData)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
            table.reload('tenderingBookList', {
                initSort: obj
                ,where: {
                    sort: obj.field //排序字段
        ...
        function getAssetsDic(id) {
            /*
             /选择设备名称
             */
            $("#bookAssetsName").bsSuggest(
                returnDicAssets(id)
            );
        }
    });
    exports('purchases/tendering/tenderingBookList', {});
});
