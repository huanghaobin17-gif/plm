layui.define(function(exports){
    layui.use(['admin', 'layer', 'form', 'formSelects', 'laydate', 'table', 'suggest', 'tablePlug'], function () {
        ...
        var gloabOptions = {};
        table.render({
            elem: '#purPlansAppLists'
            //,height: '600'
            , limits: [5, 10, 20, 50, 100]
            , loading: true
            , limit: 10
            , url: purPlansAppLists //数据接口
            , where: {
                sort: 'plans_id',
                order: 'desc'
            } //如果无需传递额外参数，可不加该参数
            ...
            , cols: [[ //表头
                ...
                {field: 'plans_year',hide: get_now_cookie(userid+cookie_url+'/plans_year')=='false'?true:false, title: '计划年份', width: 100, align: 'center'},
        ...
        //监听搜索
        form.on('submit(purPlansAppListsSearch)', function (data) {
            gloabOptions = data.field;
            //搜索时重新初始化全局搜索参数，方便修改删除用户时候刷表格面用
            //刷新表格时，默认回到第一页
            var table = layui.table;
            //table.set(gloabOptions);
            table.reload('purPlansAppLists', {
                url: purPlansAppLists
                , where: gloabOptions
                , page: {
                    curr: 1 //重新从第 1 页开始
                }
        ...
        //监听工具条
        table.on('tool(purPlansAppData)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
            var rows = obj.data; //获得当前行数据
            var layEvent = obj.event; //获得 lay-event 对应的值（也可以是表头的 event 参数对应的值）
            //var tr = obj.tr; //获得当前行 tr 的DOM对象
            var url = $(this).attr('data-url');
            if (layEvent === 'showApprove') { //查看科室计划
                top.layer.open({
                    type: 2,
                    title: '查看科室年度采购计划审批情况【' + rows.department + '】',
                    area: ['980px', '100%'],
                    offset: 'r',//弹窗位置固定在右边
                    anim: 2, //动画风格
                    scrollbar: false,
            ...
            }else if (layEvent === 'approval') { //审批计划
                var flag = 1;
                top.layer.open({
                    id: 'depart-Report',
                    type: 2,
                    title: '审批科室年度采购计划【' + rows.department + '】',
                    area: ['980px', '100%'],
                    offset: 'r',//弹窗位置固定在右边
                    anim: 2, //动画风格
                    scrollbar: false,
        ...
        //项目名称搜索建议
        $("#plans_apply_project").bsSuggest(
            returnPurPlansProjects()
        );
    });
    exports('purchases/plans/purPlansAppLists', {});
});