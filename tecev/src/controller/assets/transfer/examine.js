layui.define(function(exports){
    layui.use(['layer', 'form', 'element', 'laydate', 'table', 'suggest', 'tablePlug'], function () {
        ...
        //转科时间元素渲染
        laydate.render(dateConfig('#examineTransferdate'));

        //申请时间元素渲染
        laydate.render(dateConfig('#examineApplicantdate'));
        ...
        table.render({
            ...
            ,cols: [[ //表头
                ...
                , {
                    ...
                    title: '审核状态',
                    hide: get_now_cookie(userid+cookie_url+'/approvalStatus')=='false'?true:false,
                    width: 100,
                    fixed: 'right',
                    style: 'background-color: #f9f9f9;',
                    align:'center',
                    templet: function(d){
                        return (d.approve_status == 1) ? '<span style="color:green;">已通过</span>' : (d.approve_status == 2) ? '<span style="color:red;">不通过</span>' : (d.approve_status == 0) ? '<span style="color:#FFB800;">未审核</span>':'<span style="color:#FFB800;">不需审核</span>';
                    }
                }, {
                    field: 'checkStatus',
                    title: '验收状态',
                    hide: get_now_cookie(userid+cookie_url+'/checkStatus')=='false'?true:false,
                    width: 100,
                    fixed: 'right',
                    style: 'background-color: #f9f9f9;',
                    align:'center',
                    templet: function(d){
                        return (d.is_check == 1) ? '<span style="color:green;">已通过</span>' : (d.is_check == 2) ? '<span style="color:red;">未通过</span>' : '<span style="color:#FFB800;">未验收</span>';
                    }
                }
                , {
                    field: 'operation',
        ...
        //操作栏按钮
        table.on('tool(examine)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        ...
        //列排序
        table.on('sort(examine)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        ...
        //科室转出
        $("#examineAssetsDepartmentOut").bsSuggest(
            returnDepartment()
        );

        //科室转入
        $("#examineAssetsDepartmentIn").bsSuggest(
            returnDepartment()
        );


        //设备编号搜索建议
        $("#examineAssetsNum").bsSuggest(
    ...
    exports('assets/transfer/examine', {});
});
