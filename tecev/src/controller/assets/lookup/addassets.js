layui.define(function (exports) {
    layui.use(['form', 'layedit', 'laydate', 'upload', 'suggest', 'formSelects', 'tipsType', 'asyncFalseUpload'], function () {
        var old_dic_assets = '', old_departid = 0, old_is_subsidiary = 0;
        var form = layui.form, layer = layui.layer, layedit = layui.layedit, laydate = layui.laydate,
            formSelects = layui.formSelects, suggest = layui.suggest, upload = layui.upload, tipsType = layui.tipsType,
            asyncFalseUpload = layui.asyncFalseUpload;
        ...
        //上传设备档案按钮
        //记录已上传的文件
        var need_upload_files = new Array();
        var fileListView2 = $('#arcfileList')
            , uploadListIns2 = asyncFalseUpload.render({
            elem: '#multifile2'
            , url: 'addAssets'
            , data: {"assid": '', "type": 'archives', "action": "uploadFile", "identifier": identifier}
            , accept: 'file'
            ...
            , choose: function (obj) {
                ...
                obj.preview(function (index, file, result) {
                    ...
                    var tr = $(['<tr id="upload-' + index + '">'
                        ...
                        , '<td class="td-align-center" style="padding: 0;"><input type="text" name="archives_time[]" readonly value="" placeholder="点击选择日期" class="layui-input archives-time" style="cursor: pointer;border: none;height: 49px;"></td>'
                        , '<td class="td-align-center" style="padding: 0;"><input type="text" name="expire_time[]" readonly value="" placeholder="点击选择日期" class="layui-input expire-time" style="cursor: pointer;border: none;height: 49px;"></td>'
        ...
        //获取相应分类
        var cathResult = initsuggestCate();
        var catid_obj = $('select[name="catid"]');
        var dic_catid_obj = $('select[name="dic_catid"]');
        if (cathResult.status === 1) {
            catid_obj.html(cathResult.result);
            dic_catid_obj.html(cathResult.result);
        } else {
            layer.msg(cathResult.msg, {icon: 2, time: 3000});
            return false;
        }
        ...
        form.on('submit(addAssets)', function (data) {
            ...
            if (old_is_subsidiary === parseInt(YES_STATUS)) {
                ...
                if (!main_assid.data('id')) {
                    ...
                    params.main_assid = main_assid.attr('data-id');
        ...
        function initsuggestMainAssets() {
            $("#addMainAssetsName").bsSuggest({
                url: admin_name + "/Public/getAllAssetsSearch?type=subsidiary",
                ...
                var params = {};
                params.main_assid = $('input[name="main_assid"]').attr('data-id');
                params.action = 'getMainAssetsBasic';
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data.status === 1) {
                            $("select[name='departid']").val(data.result.departid);
                            $("select[name='ols_facid']").val(data.result.ols_facid);
                            $("select[name='ols_supid']").val(data.result.ols_supid);
                            $("select[name='ols_repid']").val(data.result.ols_repid);


                            // $("input[name='managedepart']").val(data.result.managedepart);
                            $("input[name='address']").val(data.result.address);
                            $("input[name='assetsrespon']").val(data.result.assetsrespon);
                            old_departid = parseInt(data.result.departid);
                            form.render();
                        } else {
                            layer.msg(data.msg, {icon: 2, time: 3000});
                        }
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.value.length > 0) {
                    var html = '<option value="">请先选择设备名称</option>';
...
//选择字典类型
    ...
    exports('controller/assets/lookup/addassets', {});
});
