layui.define(function(exports){
    layui.use(['layer', 'form', 'table', 'suggest', 'tablePlug'], function () {
        ...
        table.render({
            ...
            toolbar: '#LAY-Assets-Borrow-borrowRecordListBar',
            ...
            , cols:[ [ //表头
                ...
                {field: 'apply_time',hide: get_now_cookie(userid+cookie_url+'/apply_time')=='false'?true:false, title: '申请时间',sort: true, width: 150, align: 'center'},
                {field: 'borrow_in_time',hide: get_now_cookie(userid+cookie_url+'/borrow_in_time')=='false'?true:false, title: '借入时间',sort: true, width: 150, align: 'center'},
                {field: 'give_back_time',hide: get_now_cookie(userid+cookie_url+'/give_back_time')=='false'?true:false, title: '归还时间',sort: true, width: 150, align: 'center'},
                {field: 'deparment_approve',hide: get_now_cookie(userid+cookie_url+'/deparment_approve')=='false'?true:false, title: '科室审批', width: 90, align: 'center'},
                {field: 'assets_approve',hide: get_now_cookie(userid+cookie_url+'/assets_approve')=='false'?true:false, title: '设备科审核', width: 100, align: 'center'},
                {
                    field: 'operation',
                    title: '操作',
                    fixed: 'right',
        ...
        //搜索
        form.on('submit(borrowRecordListSearch)', function (data) {
            ...
            gloabOptions.apply_departid = field.apply_departid;
            gloabOptions.hospital_id = field.hospital_id;
            table.reload('borrowRecordList', {
                url: borrowRecordListUrl
                , where: gloabOptions
                , height: 'full-100' //高度最大化减去差值
                , page: {
                    curr: 1 //重新从第 1 页开始
        ...
        table.on('toolbar(borrowRecordData)', function(obj){
            var event =  obj.event,
                url = $(this).attr('data-url');
            switch(event){
                case 'batch_print_borrow':
                    var checkStatus = table.checkStatus('borrowRecordList');
                    ...
                    for(var j = 0,len=data.length; j < len; j++) {
                        if(parseInt(data[j]['status'])!==parseInt(BORROW_STATUS_COMPLETE)){
                    ...
                    params.borid = borid.substring(0, borid.length - 1);
                    $.ajax({
                        timeout: 5000,
                        type: "POST",
                        url: url,
                    ...
                    var borrow_report=$('#borrow_report');
                    borrow_report.show();
                    borrow_report.printArea();
                    borrow_report.hide();
                    break;
            }
        });
        ...
        //判断搜索建议的位置
        position = '';
        if (Math.floor($("#LAY-Assets-Borrow-borrowRecordList .layui-form-item").width() / $("#LAY-Assets-Borrow-borrowRecordList .layui-inline").width()) == 3) {
            position = '';
        } else {
            position = 1;
        }
        ...
        //设备名称 搜索建议
        $("#borrowRecordListAssetsName").bsSuggest(
            returnAssets()
        );

        //规格型号 搜索建议
        $("#borrowRecordListModel").bsSuggest(
            returnAssetsModel(position)
        );

    });
    exports('assets/borrow/borrowRecordList', {});
});