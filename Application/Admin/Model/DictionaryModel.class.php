...
class DictionaryModel extends CommonModel
{
    ...
    protected $tableName = 'dic_assets';
    ...
    public function getAllAssetsDic()
    {
        ...
        $sort = I('post.sort') ? I('post.sort') : 'dic_assid';
        $assets = I('post.assets_name');
        ...
        $total = $this->DB_get_count('dic_assets', $where);
        $dicassets = $this->DB_get_all('dic_assets', '*', $where, '', $sort . ' ' . $order, $offset . ',' . $limit);
        //查询当前用户是否有权限进行删除字典操作
        $deleDic = get_menu($this->MODULE, 'Dictionary', 'delAssetsDic');
        //查询当前用户是否有权限进行修改字典操作
        $editDic = get_menu($this->MODULE, 'Dictionary', 'editAssetsDic');
        $catname=[];
        ...
        foreach ($dicassets as $k => $v) {
            ...
            $dicassets[$k]['assets_category'] = $v['assets_category'];
            $dicassets[$k]['assets_category'] = str_replace(',', '、', $dicassets[$k]['assets_category']);

            $dicassets[$k]['category'] = $catname[$v['catid']]['category'];
    ...
    {
        $menuData = get_menu($this->MODULE, $this->Controller, 'addAssetsDic');
        if (!$menuData) {
            return array('status' => -1, 'msg' => '无权限新增字典');
        }
        $data['hospital_id'] = session('current_hospitalid');
        $data['assets'] = trim(I('post.assets'));
        $data['catid'] = I('post.catid');
        $data['assets_category'] = I('post.assets_category');
        ...
        if (!$data['assets'] || !$data['catid']) {
            return array('status' => -1, 'msg' => '必填项不能为空！');
        }
        //查询是否已存在该设备名称
        $is_exists = $this->DB_get_one('dic_assets','dic_assid',array('hospital_id'=>session('current_hospitalid'),'assets'=>$data['assets']));
        if($is_exists){
            return array('status' => -1, 'msg' => '该设备名称已存在！');
        }
        $newid = $this->insertData('dic_assets', $data);
        if ($newid) {
            return array('status' => 1, 'msg' => '添加设备字典成功！');
        } else {
            return array('status' => -1, 'msg' => '添加设备字典失败！');
        }
    }
    ...
    public function editDic()
    {
        ...
        $res = $this->updateData('dic_assets', $data, array('dic_assid' => $dic_assid));
        if ($res) {
