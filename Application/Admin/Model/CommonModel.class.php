...
class CommonModel extends Model
{
    ...
    {
        $res = M($table,
            C('DB_PREFIX'))->alias($alias)->field($fields)->join($join)->where($where)->group($group)->order($order)->limit($limit)->select();
        //echo M($table,C('DB_PREFIX'))->getLastSql();exit;
        return $res;
    }
    ...
    {
        $res = M($table,
            C('DB_PREFIX'))->alias($alias)->field($fields)->join($join)->where($where)->group($group)->order($order)->find();
        //echo M($table,C('DB_PREFIX'))->getLastSql();exit;
        return $res;
    }
    ...
    */
    ...
    {
        $res = M($table,
            C('DB_PREFIX'))->field($fields)->where($where)->group($group)->order($order)->limit($limit)->select();
//        echo M($table,C('DB_PREFIX'))->getLastSql();exit;
        return $res;
    }
    ...
    * @param1 $table string 查询表
    * @param2 $where string  查询条件
    * @param3 $field string  字段
    * @param2 $type string  修改类型 setInc自增 setDec自减
    * @param2 $num int  数量
    */
    public function DB_set_update($table, $where = '', $field, $type = 'setInc', $num = 0)
    {
        if ($type == 'setInc') {
            $res = M($table, C('DB_PREFIX'))->where($where)->setInc($field, $num);
        } else {
            $res = M($table, C('DB_PREFIX'))->where($where)->setDec($field, $num);
        }
        //echo M($table,C('DB_PREFIX'))->getLastSql();exit;
        return $res;
    }
    ...
    */
    ...
    {
        return '<button type="button" class="layui-btn ' . $class . '" lay-event="' . $layEvent . '" style="' . $style . '"  data-url="' . $url . '" ' . $string . '>' . $name . '</button>';
    }
    ...
    获取报告的标题
     */
    public function getprinttitle($model, $set_item)
    {
        $data  = $this->DB_get_one('base_setting', 'value', ['model' => $model, 'set_item' => $set_item]);
...
//        }
        $user = $this->DB_get_all('user', 'userid,username',
            ['status' => 1, 'is_super' => '0', 'is_delete' => 0, 'job_hospitalid' => session('current_hospitalid')], '',
            'userid asc', '');
        return $user;
    }
    ...
    {
        if (!$value) {
            die(json_encode(['status' => -1, 'msg' => $text]));
        }
    }
    ...
    {
        $this->insertData('assets_state_change', [
            'assid'       => $assid,
            'new_status'  => $status,
            'remark'      => $remark,
            'change_user' => session('username'),
            'change_time' => getHandleDate(time()),
        ]);
    }
    ...
    {
        $res = $this->DB_get_one('approve_type', '*',
            ['approve_type' => $type, 'hospital_id' => $hospital_id, 'status' => C('OPEN_STATUS')]);
        if ($res) {
            return true;
        } else {
            return false;
    ...
    public function checkSmsIsOpen($model)
    {
        ...
        if (!$settingData) {
            ...
            if (!$Data) {
                $Data = M('sms_basesetting')->where(['hospital_id' => session('current_hospitalid')])->cache('sms_basesetting',
                    3600)->select();
            }
            if (!$Data) {
                return false;
    ...
    public function checkApproveIsSetProcess($type, $hospital_id)
    {
        ...
        $res = $this->DB_get_one('approve_type', '*',
            ['approve_type' => $type, 'hospital_id' => $hospital_id, 'status' => C('OPEN_STATUS')]);
        if ($res) {
            $process = $this->DB_get_one('approve_process', '*', ['typeid' => $res['typeid']]);
            if ($process) {
                return true;
            } else {
                return false;
    ...
    根据用户名返回签名图片
     */
    public function get_autograph($username)
    {
        $data = $this->DB_get_one('user', 'autograph', ['username' => $username]);
        return $data['autograph'];
    }

    /*
    根据科室名返回科室负责人签名图片
     */
    public function get_departid_autograph($departid)
    {
        $join = 'LEFT JOIN sb_department AS B ON B.manager=A.username';
        $data = $this->DB_get_one_join('user', 'A', 'A.autograph', $join, ['B.departid' => $departid]);
        return $data['autograph'];
    }
    ...
    返回设备科审批人的签名图片
     */
    public function get_assets_autograph()
    {

        $join = 'LEFT JOIN sb_department AS B ON B.manager=A.username';
        $data = $this->DB_get_one_join('user', 'A', 'A.autograph', $join, ['B.department' => '设备科']);
        return $data['autograph'];
    }
    ...
    根据用户id返回签名图片
     */
    public function get_autograph_id($id)
    {
        $data = $this->DB_get_one('user', 'autograph', ['userid' => $id]);
        return $data['autograph'];
    }
    ...
    public function checkCateNum($catenum)
    {
        if (C('CATE_PREFIX_NUM') == '') {
            return ['status' => 1, 'msg' => '系统未设置分类编码规则', 'num' => $catenum];
        }
        $prefix = substr($catenum, 0, strlen(C('CATE_PREFIX_NUM')));
        if ($prefix == C('CATE_PREFIX_NUM')) {
            return ['status' => 1, 'msg' => '分类编码规则符合系统设置', 'num' => $catenum];
        } else {
            return [
                'status' => -1,
                'msg'    => '分类编码规则不符合系统设置要求，请以 ' . C('CATE_PREFIX_NUM') . ' 为开头设置分类编码！',
            ];
        }
    }
    ...
    public function checkDepartNum($departnum)
    {
        if (C('DEPART_PREFIX_NUM') != '') {
            ...
            if ($prefix == C('DEPART_PREFIX_NUM')) {
                return ['status' => 1, 'msg' => '部门编码规则符合系统设置要求', 'num' => $departnum];
            } else {
                return [
                    'status' => -1,
                    'msg'    => '部门编码规则不符合系统设置要求，请以 ' . C('DEPART_PREFIX_NUM') . ' 为开头设置部门编码！',
                ];
            }
        }
    }
    ...
    {
        return $this->DB_get_all('hospital', '*', ['is_delete' => 0]);
    }

    public function get_all_user_can_manager_hospital($userid)
    {
        $userinfo = $this->DB_get_one('user', 'manager_hospitalid', ['userid' => $userid]);
        return $this->DB_get_all('hospital', '*',
            ['is_delete' => 0, 'hospital_id' => ['in', $userinfo['manager_hospitalid']]]);
    }
    ...
    public function get_hospital_name($hospital_id)
    {
        ...
        if (count($hosidarr) > 1) {
            ...
            $res = $this->DB_get_one('hospital', '*', ['hospital_id' => $hospital_id]);
        }
        return $res;
    }
    ...
    public function check_approve_process($departid, $approve_process_user, $level)
    {
        ...
        foreach ($approve_process_user as &$approveV) {
            if ($approveV['approve_user'] == '部门审批负责人') {
                $traninManager = $this->DB_get_one('department', 'manager', ['departid' => $departid]);
                if (!$traninManager['manager']) {
                    $departname = [];
                    include APP_PATH . "Common/cache/department.cache.php";
                    die(json_encode([
                        'status' => -1,
                        'msg'    => '该设备所属科室（' . $departname[$departid]['department'] . '）未设置审批负责人，请先设置',
                    ]));
                } else {
    ...
    {
        //查询typeid
        $type                                 = $this->DB_get_one('approve_type', 'typeid',
            ['approve_type' => $approve_type, 'hospital_id' => $hospital_id]);
        $approve_process_where['typeid']      = ['EQ', $type['typeid']];
        $approve_process_where['start_price'] = ['ELT', $price];
        $approve_process_where['end_price']   = ['GT', $price];
        //查询是否有关联部门的审核流程
        $approve_process = $this->DB_get_one('approve_process', 'processid', $approve_process_where);
        if ($approve_process) {
            $approve_process_user = $this->DB_get_all('approve_process_user', 'approve_user,approve_user_aux,listorder',
                ['processid' => $approve_process['processid']], '', 'listorder ASC');
            if (!$approve_process_user) {
                die(json_encode(['status' => -1, 'msg' => '未分配审批人员，请联系管理员']));
            } else {
                $usernameArr               = [];
                ...
                if (!$approve_process_user_data) {
                    die(json_encode(['status' => -1, 'msg' => '配置的审批人停用或已删除，请联系管理员重新配置']));
                }
    ...
    public function addApprove($arr, $data, $price, $hospital_id, $departid, $approveType, $tableName, $idName)
    {
        ...
        $this->updateData($tableName, [
            'complete_approver'     => $completeornotuser['complete'],
            'not_complete_approver' => $completeornotuser['notcomplete'],
        ], [$idName => $arr[$idName]]);
        //判断是否是最后一道审批或者审批不通过
        if ($data['process_node_level'] == $lastProcess || $data['is_adopt'] == C('STATUS_APPROE_FAIL')) {
            //更新表对应记录为最后审核状态
            $this->updateData($tableName,
                ['approve_status' => $data['is_adopt'], 'approve_time' => getHandleDate(time())],
                [$idName => $arr[$idName]]);
            if ($data['is_adopt'] == C('STATUS_APPROE_SUCCESS')) {
                ...
                switch ($approveType) {
                    case C('REPAIR_APPROVE'):
                        //修改维修表中status为维修中
                        $this->updateData($tableName, ['status' => C('REPAIR_MAINTENANCE')],
                            [$idName => $arr[$idName]]);
                        //最后一次审批通过以后生成一条维修合同
                        if ($arr['repair_type'] == C('REPAIR_TYPE_THIRD_PARTY')) {
                            //第三方类型审批通过后 生成1条待确认的合同 记录基本信息
                            $companyInfo                         = $this->DB_get_one('repair_offer_company',
                                'offer_company_id,total_price,offer_company,offer_contacts,telphone',
                                ['repid' => $data['repid'], 'last_decisioin' => 1]);
                        ...
                        if ($arr['repair_type'] == C('REPAIR_TYPE_IS_STUDY')) {
                            ...
                            if ($inwareData) {
                                ...
                                $this->updateData('parts_inware_record_detail', ['is_user' => C('YES_STATUS')],
                                    $inwareWhere);
                                // 修改出库明细单状态
                                $outWhere['inware_partsid'] = ['IN', $detailid];
                                $this->updateData('parts_inware_record_detail', ['status' => C('YES_STATUS')],
                                    $outWhere);
                            }
                        }
                        //通知审批结果
                    ...
                    case C('SCRAP_APPROVE'):
                        ...
                        if ($wx_status) {
                            $this->send_wechat_scrap_news($arr, $lastProcess, $data, $departid, $approve_process_user);
                        }
                        break;
                    ...
                    case C('SUBSIDIARY_APPROVE'):
                        ...
                        break;
                    case C('PATROL_APPROVE'):
                        //本次为最后一道审批，且为同意巡查计划的
                        //修改状态patrol_status=3待发布
                        $this->updateData('patrol_plans', ['patrol_status' => 2], ['patrid' => $arr['patrid']]);
                        //$this->patrolResultSms($arr,1);
                        break;
                    case C('INVENTORY_PLAN_APPROVE'):
                        //本次为最后一道审批，且为同意盘点计划的
                        $this->updateData('inventory_plan',
                            [
                                'inventory_plan_status' => 4,
                            ],
                            [
                                'inventory_plan_id' => $arr['inventory_plan_id'],
                            ]);
                        // 如果盘盈, 新增设备并且进行报废申请
                        $join[0]               = "LEFT JOIN sb_inventory_plan AS B ON A.inventory_plan_id = B.inventory_plan_id";
                        $join[1]               = "LEFT JOIN sb_assets_info AS C ON C.assnum = A.assetnum";
                        $inventory_plan_assets = $this->DB_get_all_join('inventory_plan_assets', 'A',
                            'A.*,B.inventory_plan_no,C.assid,C.status'
                            , $join
                            , [
                                'A.inventory_plan_id' => $arr['inventory_plan_id'],
                                'A.inventory_status'  => 2,
                                'B.is_delete'         => 0,
                        ...
                        foreach ($inventory_plan_assets as $inventory_plan_asset) {
                            ...
                            $result['assets_data']['assid']  = $inventory_plan_asset['assid'];
                            $result['assets_data']['assnum'] = $inventory_plan_asset['assetsnum'];
                            $result['assets_data']['status'] = $inventory_plan_asset['status'];

                            // todo 如果报废的校验逻辑更改了，这里也要更改
                            // 校验是否可以申请报废
                            if (
                                $this->checkScrap([
                                    'assid'       => $result['assets_data']['assid'],
                                    'status'      => $result['assets_data']['status'],
                                ...
                                $tmp_post = $_POST;
                                unset($_POST);
                                $_POST['assid']        = $result['assets_data']['assid'];
                                $_POST['scrapnum']     = 'BF-' . $result['assets_data']['assnum'];
                                $_POST['scrapdate']    = date('Y-m-d');
                                $_POST['scrap_reason'] = sprintf('由盘点单%s自动生成',
                                    $inventory_plan_asset['inventory_plan_no']);
            ...
            if ($data['is_adopt'] == C('STATUS_APPROE_FAIL')) {
                ...
                if ($approveType == C('OUTSIDE_APPROVE')) {
                    //本次为最后一道审批，且为不同意外调的
                    $this->updateData('assets_outside', ['status' => C('OUTSIDE_STATUS_FAIL'), 'retrial_status' => 1],
                        ['outid' => $arr['outid']]);
                    $detail = $this->DB_get_all('assets_outside_detail', 'subsidiary_assid',
                        ['outid' => $arr['outid']]);
                    if ($detail) {
                    ...
                    //通知审批结果
                    $this->outsideResultSms($data, $arr, $departid);
                } elseif ($approveType == C('PATROL_APPROVE')) {
                    //本次为最后一道审批，且不同意巡查计划
                    $this->updateData('patrol_plans', ['is_delete' => 1], ['patrid' => $arr['patrid']]);
                    ...
                    //本次为最后一道审批，且为不同意报废的
                    $this->updateData('assets_scrap',
                        ['approve_status' => C('STATUS_APPROE_FAIL'), 'retrial_status' => 1],
                        ['scrid' => $arr['scrid']]);
                    $detail = $this->DB_get_all('assets_scrap_detail', 'subsidiary_assid', ['scrid' => $arr['scrid']]);
                    if ($detail) {
                        foreach ($detail as &$detailV) {
                            $change_assid[] = $detailV['subsidiary_assid'];
                        }
                    ...
                    $this->updateData('subsidiary_allot', $changeData, ['allotid' => $arr['allotid']]);
                    ...
                    if ($arr['repair_type'] == C('REPAIR_TYPE_IS_STUDY')) {
                        ...
                        $inwareWhere['status'] = ['EQ', C('YES_STATUS')];
                        $inwareWhere['is_use'] = ['EQ', C('NO_STATUS')];
                        $inwareData            = $this->DB_get_all('parts_inware_record_detail', 'detailid,inwareid',
                            $inwareWhere);
                        if ($inwareData) {
                            $detailid = [];
                            $inwareid = [];
                            ...
                            $this->updateData('parts_outware_record_detail', ['status' => C('STATUS_APPROE_FAIL')],
                                $outDetailWhere);
                        }
                        //不同意，更新设备最新状态为在用
                        $this->updateData('assets_info', ['status' => C('ASSETS_STATUS_USE')],
                            ['assid' => $arr['assid']]);
                    }
                    //通知最后审批结果
                    $this->repairResultSms($data, $arr, $departid);
                } elseif ($approveType == C('INVENTORY_PLAN_APPROVE')) {
                    //盘点审核不通过
                    $inventory_plan                   = $arr;
                    $inwareWhere['inventory_plan_id'] = $inventory_plan['inventory_plan_id'];
                    $updateData                       = [
                        'inventory_plan_status' => 4,
                    ];
                    $this->updateData('inventory_plan', $updateData, $inwareWhere);

                } else {
                    //不同意，更新设备最新状态为在用
                    $this->updateData('assets_outside', ['status' => C('ASSETS_STATUS_USE')],
                        ['assid' => $arr['assid']]);
                }
                //记录设备变更信息
            ...
            //获取下次审批人  当前level+1
            $approve                      = $this->check_approve_process($departid, $approve_process_user,
                $data['process_node_level'] + 1);
            $saveData['current_approver'] = $approve['current_approver'];
            $this->updateData($tableName, $saveData, [$idName => $arr[$idName]]);
            $approve_user = $approve['this_current_approver'];
            $this->noticeNextApprovalSms($approveType, $approve_user, $data, $arr, $departid);
            if ($wx_status && $approveType == C('SCRAP_APPROVE')) {
                $this->send_wechat_scrap_news($arr, $lastProcess, $data, $departid, $approve_process_user);
            }
        }
        return ['status' => 1, 'msg' => '审批成功！', 'lastSql' => $lastSql];
    }
     ...
     * @return bool
     */
    public function checkScrap($assets)
    {
        //筛选计量计划中
        $meteringWhere['status'] = ['EQ', C('YES_STATUS')];
        $meteringWhere['assid']  = ['EQ', $assets['assid']];
        ...
        if ($this->checkApproveIsOpen(C('SCRAP_APPROVE'), $assets['hospital_id'])) {
            if (!$this->checkApproveIsSetProcess(C('SCRAP_APPROVE'), $assets['hospital_id'])) {
                return false;
            }
        }
        return true;
    ...
    {
        //修改报废单状态
        $this->updateData('assets_scrap', ['approve_status' => C('STATUS_APPROE_SUCCESS')], ['scrid' => $arr['scrid']]);
        ...
        //更新设备最新状态为已报废
        $this->updateData('assets_info', ['status' => C('ASSETS_STATUS_SCRAP')], ['assid' => ['IN', $change_assid]]);
        //记录设备变更信息
    ...
    {
        //修改 外调单状态
        $this->updateData('assets_outside', ['status' => C('OUTSIDE_STATUS_ACCEPTANCE_CHECK')],
            ['outid' => $arr['outid']]);
        $all_subsidiaryWhere['main_assid']  = ['EQ', $arr['assid']];
        $all_subsidiaryWhere['status'][0]   = 'NOTIN';
        $all_subsidiaryWhere['status'][1][] = C('ASSETS_STATUS_SCRAP');//报废
        ...
        if ($all_subsidiaryData) {
            ...
            //获取选中外调的附属设备id
            $detail = $this->DB_get_all('assets_outside_detail', 'subsidiary_assid', ['outid' => $arr['outid']]);
            if ($detail) {
                $subsidiary_assid = [];
                foreach ($detail as &$detailV) {
                    $subsidiary_assid[] = $detailV['subsidiary_assid'];
        ...
        //更新设备最新状态为已外调
        $this->updateData('assets_info', ['status' => C('ASSETS_STATUS_OUTSIDE')], ['assid' => ['IN', $change_assid]]);
        //记录设备变更信息
    ...
    {
        //更新部门表中每个部门的设备数量和总价
        $departCount = $this->DB_get_all('assets_info',
        ...
        //更新分类表中每个分类的设备数量和总价
        $cateCount = $this->DB_get_all('assets_info', 'catid,sum(buy_price) as totalPrice,count(catid) as assetsNum',
            '', 'catid', '');
        ...
        //获取父分类信息
        $cateParent = $this->DB_get_one('category', 'group_concat(catid) as ids', ['parentid' => 0, 'is_delete' => 0]);
        if ($cateParent['ids']) {
            $parentcount = $this->DB_get_all('category',
    ...
    {
        return $this->DB_get_all('department', 'departid,department',
            ['hospital_id' => $hospital_id, 'is_delete' => C('NO_STATUS')]);
    }
    ...
    public function getAreas($cityid = '')
    {
        ...
        return ['status' => 1, 'msg' => '获取成功', 'result' => $area];
    }
    ...
    public function formatPhone($userData)
    {
        ...
        foreach ($userData as &$one) {
            if ($one['username'] != session('username')) {
                $phone .= ',' . $one['telephone'];
            }
        }
        $phone = ltrim($phone, ",");
     ...
     * @return string
     */
    public function createCodePic($string, $extra_name = '')
    {
        ...
        //文件名
        $filename = $extra_name . date('YmdHis') . '.png';
        //生成二维码,第二个参数为二维码保存路径
    ...
    public function outsideResultSms($data, $outsideInfo, $departid)
    {
        ...
        if ($settingData) {
            ...
            if ($settingData['outsideApproveOver']['status'] == C('OPEN_STATUS') && $approve_user['telephone']) {
                $sms = AssetsOutsideModel::formatSmsContent($settingData['outsideApproveOver']['content'],
                    $outsideInfo);
        ...
        if ($settingData) {
            ...
            if ($data['is_adopt'] == C('STATUS_APPROE_FAIL')) {
                ...
                if ($settingData['subsidiaryApproveOver']['status'] == C('OPEN_STATUS') && $approve_user['telephone']) {
                    $sms = SubsidiaryModel::formatSmsContent($settingData['subsidiaryApproveOver']['content'],
                        $assInfo);
    ...
    public function scrapResultSms($data, $status)
    {
        ...
        if ($settingData) {
            //有开启短信
            $scrapInfo            = $this->DB_get_one('assets_scrap', 'scrapnum,assid,apply_user',
                ['scrid' => $data['scrapid']]);
            $asInfo               = $this->DB_get_one('assets_info', 'assnum,assets', ['assid' => $scrapInfo['assid']]);
            ...
            $telephone = $this->DB_get_one('user', 'telephone', ['username' => $scrapInfo['apply_user']]);
     ...
     * @param $status 1通过2不通过
     */
    public function patrolResultSms($arr, $status)
    {
        //==========================================短信 START==========================================
        $scrapModel  = new AssetsScrapModel();
        ...
        if ($settingData) {
            ...
            $ToolMod   = new ToolController();
            $telephone = $this->DB_get_one('user', 'telephone', ['username' => $scrapInfo['apply_user']]);
            $sms       = $scrapModel->formatSmsContent($settingData['approvePatrolStatus']['content'], $smsData);
            $ToolMod->sendingSMS($telephone['telephone'], $sms, 'Patrol', $arr['patrid']);
            //==========================================短信 END==========================================
        }
    }
...
//        }
     ...
     * @param string $otherStr 传入其他字符串
     *
     * @return  string
     * */
    public function getOrgNumber($table, $field, $prefix = '', $cut = '', $otherStr = '')
    {
        if ($otherStr) {
            $thisDate = $data['repnum'] = date('Ymd', $otherStr);
            $last     = $this->DB_get_one($table, $field . ' AS num', '', 'repid DESC');
        } else {
            $thisDate = $data['repnum'] = date('Ymd', time());
            $last     = $this->DB_get_one($table, $field . ' AS num', '', $field . ' DESC');
        }
        if ($last) {
            $prefixLenght = strlen($prefix);
            $cutLenght    = strlen($cut);
            //判断是否是今年的
            $lastYear = substr($last['num'], $prefixLenght, 4);
            if ($lastYear == substr($thisDate, 0, 4)) {
                $lastNum = substr($last['num'], $prefixLenght + 8 + $cutLenght);
                $num     = $prefix . $thisDate . $cut . sprintf("%04d", ++$lastNum);
            } else {
                $num = $prefix . $thisDate . $cut . sprintf("%04d", 1);
            }
        } else {
            $num = $prefix . $thisDate . $cut . sprintf("%04d", 1);
        }
    ...
    public function get_department_engineers($arr_departids, $hospital_id = 0)
    {
        $users = [];
        foreach ($arr_departids as $v) {
            $uuid = session('userid') ? session('userid') : 0;
            $res  = $this->getToUser($uuid, $v, 'Patrol', 'Patrol', 'doTask', $hospital_id);
            foreach ($res as $rv) {
                $users[] = $rv;
            }
        }
    ...
    //发送微信消息(一个或多个)
    public function sendMesByWechat(
        $uid,
        $departid,
        $moduleName,
        $controllerName,
        ...
        $toUser       = $this->getToUser($uid, $departid, $moduleName, $controllerName, $actionName);
        ...
        foreach ($toUser as $k => $v) {
            ...
            if ($k == 0) {
                //是否开启反向代理
                if (!C('OPEN_AGENT')) {
                    $res = dcurl($wxurl, $template);
                    $res = json_decode($res);
                } else {
                    $res = dcurl(C('SEND_MSG_URL') . '?access_token=' . $access_token, $template);
                    $res = json_decode($res);
                }
                if ($res->errcode != 0) {
                    ...
                    if (!C('OPEN_AGENT')) {
                        $wxurl = "https://api.weixin.qq.com/cgi-bin/message/template/send?access_token=" . $access_token;
                        dcurl($wxurl, $template);
                    } else {
                        dcurl(C('SEND_MSG_URL') . '?access_token=' . $access_token, $template);
                    }
                }
                $alerady_send[] = $v['openid'];
            } elseif ($v['openid'] && !in_array($v['openid'], $alerady_send)) {
    ...
    //发送微信消息给特定用户
    public function sendMsgToOnUserByWechat($openid, $templateId, $redect_url, $data)
    {
        if ($openid) {
            static $num = 0;
            $wxModel      = new WxAccessTokenModel();
            ...
            $template     = urldecode($template);
            if ($num == 0) {
                //第一次用curl发送，以便判断是否发送成功，发送不成功则更新token
                //是否开启反向代理
                if (!C('OPEN_AGENT')) {
                    $res = dcurl($wxurl, $template);
                    $res = json_decode($res);
                ...
                if ($res->errcode != 0) {
                    ...
                    if ($num == 1) {
                        ...
                        $map['item'] = 'access_token';
                        $wx->where($map)->delete();
                        $this->sendMsgToOnUserByWechat($openid, $templateId, $redect_url, $data);
                    }
                }
            } else {
                //不是第一次直接发送
    ...
    //获取要发送微信消息的用户openid
    public function getToUser($uid, $departid, $moduleName, $controllerName, $actionName, $hospital_id = 0)
    {
        //查询对应操作的menuid
        $menu_where['status'] = 1;
        $menu_where['name']   = [['eq', $moduleName], ['eq', $controllerName], ['eq', $actionName], 'or'];
        $menus                = $this->DB_get_all('menu', 'menuid,name,title,parentid', $menu_where, '',
            'parentid asc');
        $menuid               = 0;
        ...
        $role_where['B.hospital_id'] = $hospital_id ? $hospital_id : session('current_hospitalid');
        $role                        = $this->DB_get_one_join('role_menu', 'A', 'group_concat(A.roleid) as roleids',
            'LEFT JOIN sb_role AS B ON A.roleid = B.roleid', $role_where);
        ...
        //$join[1] = " LEFT JOIN sb_user AS C ON A.userid = C.userid and C.is_delete = 0 and C.status = 1";
        $join[1]              = " LEFT JOIN sb_user AS C ON A.userid = C.userid";
        $fields               = "A.*,B.departid,C.userid,C.username,C.openid,C.autograph,C.telephone";
        $where['A.roleid']    = ['in', $role['roleids']];
        $where['A.userid']    = ['neq', $uid];
        $where['C.is_delete'] = 0;
        $where['C.status']    = 1;
        $where['C.openid']    = ['exp', 'is not null'];
        $where['C.openid']    = ['neq', ''];
        ...
        $SM4 = new \SM4();
        foreach ($res as &$v) {
            $v['telephone'] = strlen($v['telephone']) > 12 ? $SM4->decrypt($v['telephone']) : $v['telephone'];
        }
        return $res;
    }

    //发送报废微信消息
    public function send_wechat_scrap_news($arr, $lastProcess, $data, $departid, $approve_process_user)
    {
        $departname = [];
        include APP_PATH . "Common/cache/department.cache.php";
        $assInfo               = $this->DB_get_one('assets_info', 'assid,assets,assnum,departid',
            ['assid' => $arr['assid']]);
        $assInfo['department'] = $departname[$assInfo['departid']]['department'];
        //判断是否是最后一道审批或者审批不通过
        if ($data['process_node_level'] == $lastProcess || $data['is_adopt'] == C('STATUS_APPROE_FAIL')) {
            $username = $this->DB_get_one('user', 'telephone,openid', ['username' => $arr['add_user']]);
            if ($data['is_adopt'] == C('STATUS_APPROE_FAIL')) {
                //审批不通过通知报废者
                if ($username['openid']) {
                    if (C('USE_VUE_WECHAT_VERSION')) {
                        $redecturl = C('APP_NAME') . C('VUE_FOLDER_NAME') . '/#' . C('VUE_NAME') . '/Scrap/applyScrap?scrid=' . $arr['scrid'];
                    } else {
                        $redecturl = C('HTTP_HOST') . C('MOBILE_NAME') . '/Scrap/applyScrap.html?scrid=' . $arr['scrid'];
                    }

                    Weixin::instance()->sendMessage($username['openid'], '设备报废审核结果通知', [
                        'thing5'            => $assInfo['department'],// 所属科室
                        'thing1'            => $assInfo['assets'],// 设备名称
                        'character_string6' => $assInfo['assnum'],// 设备编码
                        'character_string7' => $arr['scrapnum'],// 报废单号
                        'const2'            => '未通过',// 审核结果
                    ], $redecturl);
                }
            } else {
            ...
            //获取下一个审批人
            $next_approver = $this->check_approve_process($departid, $approve_process_user,
                $data['process_node_level'] + 1);
            //审批通过通知下一位
            $username = $this->DB_get_one('user', 'openid', ['username' => $next_approver['this_current_approver']]);
            if ($username['openid']) {
                if (C('USE_VUE_WECHAT_VERSION')) {
                    $redecturl = C('APP_NAME') . C('VUE_FOLDER_NAME') . '/#' . C('VUE_NAME') . '/Scrap/examine?scrid=' . $arr['scrid'];
                } else {
                    $redecturl = C('HTTP_HOST') . C('MOBILE_NAME') . '/Scrap/examine.html?scrid=' . $arr['scrid'];
                }

                Weixin::instance()->sendMessage($username['openid'], '设备报废待审批提醒', [
                    'thing8'            => $assInfo['department'],// 所属科室
                    'thing1'            => $assInfo['assets'],// 设备名称
                    'character_string9' => $assInfo['assnum'],// 设备编码
                    'character_string7' => $arr['scrapnum'],// 报废单号
                    'thing2'            => $arr['add_user'],// 申请人
                ], $redecturl);
            }
        }
    }
    ...
    public function get_delete_departids()
    {
        $hospital_id = session('current_hospitalid');
        $ids         = $this->DB_get_one('department', 'group_concat(departid) as ids',
            ['is_delete' => 1, 'hospital_id' => $hospital_id]);
        return $ids['ids'];
    }
    ...
    public function get_approves_progress($data, $app_id, $data_id)
    {
        if ($data['all_approver']) {
            ...
            $apps = $this->DB_get_all('approve',
                $app_id . ',group_concat(is_adopt ORDER BY apprid ASC) as is_adopt,group_concat(approve_time ORDER BY apprid ASC) as approve_time',
                ['is_delete' => 0, $app_id => $data[$data_id]], $app_id);
            //查询系统超级用户
