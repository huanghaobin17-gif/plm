...
namespace Vue\Controller\Pub;

use Admin\Controller\NotCheckLogin\PublicController;
use Admin\Model\AssetsInfoModel;
use Admin\Model\ModuleModel;
use Vue\Model\AssetsBorrowModel;
use Vue\Model\AssetsTransferModel;
use Vue\Model\QualityModel;
use Vue\Model\RepairModel;
use Vue\Model\AssetsScrapModel;
...
class NotinController extends Controller
{
    ...
    public function getUserOpenId()
    {
        if (I('GET.code')) {
            ...
            if ($content['openid'] != null) {
                ...
                $wxModel = new \Vue\Model\WxAccessTokenModel();
                ...
                if ($snsapi_userinfo['errcode']) {
                    $result['tips'] = '获取用户信息失败，errcode：'.$snsapi_userinfo['errcode'].'，errmsg：'.$snsapi_userinfo['errmsg'];
                    $result['btn'] = '';
                    $result['url'] = '';
                    $result = json_encode($result, JSON_UNESCAPED_UNICODE);
                    redirect(C('VUE_FOLDER_NAME') . '/#/fail/' . $result);
                ...
                if (!$users) {
                    //查找不到该openid的用户
                    redirect(C('VUE_FOLDER_NAME') . '/#/login');
                    exit;
                }
                if (count($users) == 1) {
                    ...
                    if ($content['openid'] == $old_session_openid && session('userid')) {
                        //直接进入首页
                        redirect(C('VUE_FOLDER_NAME') . '/#/');
                        exit;
                    } else {
                        redirect(C('VUE_FOLDER_NAME') . '/#/chan_user');
                        exit;
                    }
                }
            } else {
                ...
                $result['btn'] = '重新授权';
                $result['url'] = C('VUE_NAME') . '/Notin/getUserOpenId';
                $result = json_encode($result, JSON_UNESCAPED_UNICODE);
                redirect(C('VUE_FOLDER_NAME') . '/#/fail/' . $result);
                exit;
            }
    ...
    public function setSession($userid, $returnJosn = 0)
    {
        ...
        if ($res['status'] == -1) {
            ...
            if ($pre_url) {
                ...
                redirect(C('VUE_FOLDER_NAME') . '/#/'.$pre_url);
            } else {
                if ($returnJosn) {
                    $this->ajaxReturn($res);
                } else {
    ...
    public function fail()
    {
        ...
        $tips = I('GET.tips');
        $btn = I('GET.btn');
        $this->assign('url', C('VUE_NAME') . '/' . $redirectUrl);
        $this->assign('tips', $tips);
        $this->assign('btn', $btn);
        $this->display();
    }
    ...
    {
        if (!$_SESSION['userid']) {
            redirect(C('VUE_NAME')."/Notin/getUserOpenId");
            exit;
        }
        $num = 8;//轮播每页8个
        //设备管理
    ...
    private function get_assets_menus($num)
    {
        ...
        if ($jdyq) {
            ...
            $jdyq['actionurl'] = C('VUE_NAME').'/Borrow/giveBackCheckList/showReminderList';
    ...
    private function get_repair_menus($num)
    {
        ...
        if ($smjx) {
            ...
            $smjx['actionurl'] = C('VUE_NAME').'/Repair/overhaulLists';
    ...
    private function get_kuaijie()
    {
        ...
        if ($sp || $trans || $scrap || $borrow) {
            ...
            $sp['actionname'] = '审批';
            $sp['actionurl'] = C('VUE_NAME').'/Notin/approve';
            //统计待审批数量
        ...
        if ($ys_Repair || $ys_Transfer || $ys_inBorrow || $ys_giveBorrow) {
            ...
            $ys['actionname'] = '验收';
            $ys['actionurl'] = C('VUE_NAME').'/Notin/check';
            //统计待验收数量
        ...
        if ($progress_Repair || $progress_Transfer || $progress_Borrow) {
            ...
            $progress['actionurl'] = C('VUE_NAME').'/Notin/progress';
            //统计待验收数量
        ...
        if (1) {
            ...
            $patrol['actionurl'] = C('VUE_NAME').'/Patrol/patrolList';
            $menus[] = $patrol;
        }
        //公告
        ...
        if (1) {
            ...
            $edit['icon'] = C('APP_NAME') . '/Public/mobile/images/icon/paword.png';
            $edit['actionurl'] = C('VUE_NAME').'/Notin/edit-password?userid='.session('userid').'&username='.session('username');
            $menus[] = $edit;
        }
        if (count($menus) >= 9) {
            //更多
...
//            }
    ...
    public function todo()
    {
        ...
        $assnum = I('get.assnum');
        $asModel = new \Vue\Model\AssetsInfoModel();
    ...
    public function editPassword()
    {
        ...
        $pi_key = openssl_pkey_get_private($privkey);
        if ($pi_key === false) {
            return array('status' => -1, 'msg' => '验证错误，请联系管理员');
        }
        $pw = I('post.password');
        ...
        if (I('post.new_password') != I('post.confirm_password')) {
            $this->ajaxReturn(array('status' => -1, 'msg' => '确认密码与新密码不一致！'));
        }
        //判断密码复杂度和有效期
        if (!preg_match_all('/^(?![a-z]+$)(?![A-Z]+$)(?![0-9]+$)(?![\W_]+$)[a-zA-Z0-9\W_]{8,30}$/',I('post.new_password'))) {
            $this->ajaxReturn(array('status' => -1, 'msg' => '密码复杂度不够，长度必须为8到18位，且大写字母 小写字母 数字 特殊字符，四种包括两种，且不能出现空格'),'json');
        }
        $edit_user['password'] = $SM4->encrypt(I('POST.new_password'));
        $edit_user['set_password_time'] = date('Y-m-d H:i:s');
        $userModel->updateData('user',$edit_user,['userid'=>I('post.userid')]);
        $this->ajaxReturn(array('status' => 1, 'msg' => '修改密码成功，请重新登录'),'json');
    }
}
