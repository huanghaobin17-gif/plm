...
class RepairPartsModel extends CommonModel
{
    ...
    public function get_parts_orders()
    {
        ...
        foreach ($asArr as $k => $v) {
            ...
            if ($menuData) {
                $asArr[$k]['url'] = $this->returnMobileLink('处理', $menuData['actionurl'].'?repid='.$v['repid'], ' layui-btn');
            } else {
                $asArr[$k]['url'] = $this->returnMobileLink('处理', 'javascript:;', ' layui-btn layui-btn-disabled');
            }
        }
        if($sort == 'parts_num'){
            //配件数量排序
    ...
    public function get_parts_stock()
    {
            ...
            foreach ($data as $kk=>$vv){
                if($v['parts'] == $vv['parts'] && $v['part_model'] == $vv['parts_model'] && $v['adduser'] == $vv['leader'] && $v['repid'] == $vv['repid']){
                    ...
                    if($vv['stock_nums'] < $v['part_num']){
                        $rurl = C('MOBILE_NAME').'/RepairParts/partsInWare.html?action=partsInWareApply&repid='.$vv['repid'].'&sname='.$vv['supplier_name'].'&sid='.$vv['supplier_id'].'&stock='.$vv['stock_nums'].'&parts='.$vv['parts'].'&model='.$vv['parts_model'].'&price='.$vv['price'];
                        $data[$kk]['cus_sort'] = 2;
                        $data[$kk]['repnum'] = '（'.$v['repnum'].'）';
                        $data[$kk]['need_nums'] = '（需求量：'.$v['part_num'].$vv["unit"].'）';
                        $data[$kk]['operation'] = $this->returnMobileLink('补充库存',$rurl,'layui-btn layui-btn-danger');
                        $data[$kk]['content_name'] = 'content_red';
                    }else{
                        $rurl = C('MOBILE_NAME').'/RepairParts/partsInWare.html?action=partsInWareApply&repid='.$v['repid'];
                        $data[$kk]['cus_sort'] = 1;
                        $data[$kk]['repnum'] = '（'.$v['repnum'].'）';
                        $data[$kk]['need_nums'] = '（需求量：'.$v['part_num'].$vv["unit"].'）';
                        $data[$kk]['operation'] = $this->returnMobileLink('有待出库','','layui-btn layui-btn-normal');
        ...
        foreach ($repairparts as $k=>$v){
            $rurl = C('MOBILE_NAME').'/RepairParts/partsInWare.html?action=partsInWareApply&repid='.$v['repid'].'&sname='.$supplier_name_arr[$v['parts']][$v['part_model']].'&sid='.$supplier_id_arr[$v['parts']][$v['part_model']].'&stock=0&parts='.$v['parts'].'&model='.$v['part_model'].'&price='.$price_arr[$v['parts']][$v['part_model']];
            $v['cus_sort'] = 2;
            $v['repnum'] = '（'.$v['repnum'].'）';
            ...
            $v['stock_nums'] = 0;
            $v['unit'] = '';
            $v['operation'] = $this->returnMobileLink('补充库存',$rurl,'layui-btn layui-btn-danger');
            $v['content_name'] = 'content_red';
            array_unshift($data,$v);
        }
        $order = I('post.order') ? I('post.order') : 'DESC';
        $sort = I('post.sort') ? I('post.sort') : 'leader';
        switch ($sort){
            case 'price':
                //配件单价排序
                if($order == 'DESC'){
