...
class AssetsTransferModel extends CommonModel
{
            ...
            switch ($v['status']) {
                case C('ASSETS_STATUS_USE'):
                    ...
                    if(in_array($v['assid'],$meterassids)){
                        //计量计划中
                        $v['url'] = $this->returnMobileLink('计量计划中', 'javascript:void(0);', ' layui-btn-disabled');
                    }elseif ($borrowAssid[$v['assid']]) {
                        $v['url'] = $this->returnMobileLink('借调中', 'javascript:void(0);',  ' layui-btn-disabled');
                    } else {
                        if ($menuData) {
                            $v['url'] = $this->returnMobileLink('申请转科', $menuData['actionurl'].'?assnum='.$v['assnum'], '');
                        } else {
                ...
                case C('ASSETS_STATUS_TRANSFER_ON'):
                    ...
                    $v['url'] = $this->returnMobileLink('转科中', 'javascript:void(0);',' layui-btn-primary');
                    break;
                default:
                    $v['status_name'] = '未知状态';
                    break;
    ...
    public function get_transfer_checklist()
    {
        ...
        if (!session('isSuper')) {
            $where['B.status'] = array('NEQ', C('ASSETS_STATUS_SCRAP'));
