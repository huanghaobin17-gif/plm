...
class QualityModel extends CommonModel
{
    ...
    public function get_quality_detail_lists()
    {
        ...
        $fields = "A.qsid,A.plan_name,A.username,A.is_cycle,A.period,A.is_start,A.do_date,A.addtime,A.start_date,B.assets,B.assnum,B.departid,B.model,C.department,A.keepdata";
        ...
        foreach ($plans as &$value){
            if ($value['keepdata']) {
                $value['operation'] = $this->returnMobileLink('继续', $setQualityDetail['actionurl'] . '?qsid=' . $value['qsid'] , ' layui-btn-warm setQualityDetail');
            }else{
                $value['operation'] = $this->returnMobileLink('检测', $setQualityDetail['actionurl'] . '?qsid=' . $value['qsid'] , ' layui-btn-normal setQualityDetail');
            }
            if($value['is_cycle'] == 1){
    ...
    public function get_quality_result()
    {
        ...
        $where['B.is_delete'] = '0';
        $join = "LEFT JOIN sb_assets_info AS B ON A.assid = B.assid LEFT JOIN sb_department AS C ON B.departid = C.departid";
        $fields = "A.qsid,A.plan_name,A.username,A.is_cycle,A.period,A.is_start,A.do_date,A.addtime,A.start_date,B.assets,B.assnum,B.departid,B.model,C.department";
        $total = $this->DB_get_count_join('quality_starts', 'A', $join, $where,'');
        $plans = $this->DB_get_all_join('quality_starts', 'A', $fields, $join, $where, '', $sort . ' ' . $order, $offset . "," . $limit);
        foreach ($plans as &$value){
            $value['operation'] = $this->returnMobileLink('查看', C('MOBILE_NAME').'/Quality/showDetail?qsid=' . $value['qsid'] , ' layui-btn-normal showDetail');
            if($value['is_cycle'] == 1){
                $value['cycle_status'] = '<span style="color:#009688;">是（第 '.$value['period'].' 期）</span>';
            }else{
                $value['cycle_status'] = '<span style="color: red;">否</span>';
