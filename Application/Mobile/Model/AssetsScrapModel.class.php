            ...
            switch ($v['status']) {
                case C('ASSETS_STATUS_USE'):
                    ...
                    if (in_array($v['assid'], $meterassids)) {
                        //计量计划中
                        $v['url'] = $this->returnMobileLink('计量计划中', 'javascript:void(0);', ' layui-btn-disabled layui-btn-sm');
                    } elseif ($borrowAssid[$v['assid']]) {
                        $v['url'] = $this->returnMobileLink('借调中', 'javascript:void(0);', ' layui-btn-disabled layui-btn-sm');
                    } else {
                        if ($menuData) {
                            $v['url'] = $this->returnMobileLink('申请报废', $menuData['actionurl'] . '?assnum=' . $v['assnum'], ' layui-btn-sm');
                        } else {
//                            $v['url'] = $this->returnMobileLink('申请转科', '', ' layui-btn-disabled');
                ...
                case C('ASSETS_STATUS_SCRAP_ON'):
                    ...
                    $v['url'] = $this->returnMobileLink('报废中', 'javascript:void(0);', ' layui-btn-primary layui-btn-sm');
                    break;
                default:
                    $v['status_name'] = '未知状态';
                    break;
    ...
    public function get_scrap_examine()
    {
        ...
        foreach ($examine as $key => $value) {
            ...
            if ($value['approve_status']==0&&in_array(session('username'), $current_approver)) {
                $examine[$key]['Sort'] = 1;
                $examine[$key]['url'] = $this->returnMobileLink('审批', $menu['actionurl'] . '?scrid=' . $value['scrid'], ' layui-btn-danger');
                ...
                $total--;
                $examine[$key]['Sort'] = 2;
                $examine[$key]['url'] = $this->returnMobileLink('待审批', 'javascript:void(0);', ' layui-btn layui-btn-primary');
            }else if ($value['approve_status']==0&&!in_array(session('username'), $current_approver)&&!in_array(session('username'), $not_complete_approver)) {
                # 不是当前审批人且未通过且已审批
                $total--;
                $examine[$key]['Sort'] = 2;
                $examine[$key]['url'] = $this->returnMobileLink('已审批', 'javascript:void(0);', ' layui-btn layui-btn-primary');
            }else if ($value['approve_status']==1) {
                # 已经通过
                $total--;
                $examine[$key]['Sort'] = 2;
                $examine[$key]['url'] = $this->returnMobileLink('已通过', 'javascript:void(0);', ' layui-btn layui-btn-primary');
            }else if ($value['approve_status']==2) {
                # 已经通过
                $total--;
                $examine[$key]['Sort'] = 2;
                $examine[$key]['url'] = $this->returnMobileLink('不通过', 'javascript:void(0);', ' layui-btn layui-btn-primary');
            }else{
                $examine[$key]['url'] = $this->returnMobileLink($menu['actionname'], $menu['actionurl'] . '?scrid=' . $value['scrid'], ' layui-btn-danger');
            }
        }
