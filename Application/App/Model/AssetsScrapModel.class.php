...
class AssetsScrapModel extends CommonModel
{
    ...
    public function get_Apply_List()
    {
        ...
        $meteringWhere['hospital_id'] = UserInfo::getInstance()->get('current_hospitalid');
        ...
        $borrowWhere['status'] = [
            'IN',
            [C('BORROW_STATUS_APPROVE'), C('BORROW_STATUS_BORROW_IN'), C('BORROW_STATUS_GIVE_BACK')],
        ];
        ...
        //查询当前用户是否有权限申请报废
        $menuData = get_menu_app('Assets', 'Scrap', 'applyScrap');
        foreach ($assets as &$v) {
            if ($v['pic_url']) {
...
//                            $v['url'] = $this->returnMobileLink('测试报废', 'applyScrap?assnum=' . $v['assnum'], '');
    ...
    public function get_assets_info($assnum)
    {
        ...
        $assets = $this->DB_get_one('assets_info', '', ['assnum' => $assnum, 'is_delete' => '0']);
        if (!$assets) {
            //查询原编码
            $assets = $this->DB_get_one('assets_info', '', ['assorignum' => $assnum, 'is_delete' => '0']);
        }
        if (!$assets) {
            //查询原编码(备注)
            $assets = $this->DB_get_one('assets_info', '', ['assorignum_spare' => $assnum, 'is_delete' => '0']);
        }
        //第一分类信息
    ...
    public function get_transfer_progress()
    {
        ...
        $asArr   = $asModel->DB_get_all_join('assets_transfer', 'A', $fields, $join, $where, '', $sort . ' ' . $order,
            $offset . "," . $limit);
        if (!$asArr) {
            $result['msg']    = '暂无相关数据';
            $result['status'] = 1;
    ...
    public function get_transfer_examine()
    {
        ...
        //查询当前用户是否有权审批
        $canApproval = get_menu_app('Assets', 'Transfer', 'approval');
                ...
                if ($current_approver_arr[UserInfo::getInstance()->get('username')]) {
                    if ($canApproval) {
                        $asArr[$k]['url'] = $this->returnMobileLink('审批',
                            $canApproval['actionurl'] . '?atid=' . $v['atid'], ' layui-btn-danger');
                    } else {
                        $asArr[$k]['url'] = $this->returnMobileLink('审批', 'javascript:void(0);',
                            ' layui-btn-disabled');
                    }
                } else {
                    ...
                    if (!in_array(UserInfo::getInstance()->get('username'),
                        ...
                        //完全未审
                        $asArr[$k]['url'] = $this->returnMobileLink('待审批', 'javascript:void(0);',
                            'layui-btn-disabled');
                        ...
                        $asArr[$k]['url'] = $this->returnMobileLink('审批', 'javascript:void(0);',
                            ...
                            $complete) && !in_array(UserInfo::getInstance()->get('username'), $notcomplete)) {
                        //全部已审
                        $asArr[$k]['url'] = $this->returnMobileLink('已审核', 'javascript:void(0);',
    ...
    public function get_scrap_resultList()
    {
        ...
        //查询当前用户是否有权验收
        $canCheck   = get_menu_app('Assets', 'Transfer', 'check');
        $departname = [];
        include APP_PATH . "Common/cache/department.cache.php";
        //查询是否开启了转科审批
        foreach ($asArr as $k => $v) {
            //查询用户头像
            $uinfo                            = $this->DB_get_one('user', 'pic', ['username' => $v['applicant_user']]);
    ...
    public function get_scrap_examine()
    {
        ...
        $examine                   = $this->DB_get_all_join('assets_scrap', 'A', $fields, $join, $where, '',
            $sort . ' ' . $order, '');
        if (!$examine) {
        ...
        foreach ($examine as $key => $value) {
            ...
            if ($value['approve_status'] == 0 && in_array(UserInfo::getInstance()->get('username'),
                ...
                $examine[$key]['Sort'] = 1;
                $examine[$key]['btn']  = "审批";
            } elseif ($value['approve_status'] == 0 && !in_array(UserInfo::getInstance()->get('username'),
                    $current_approver) && in_array(UserInfo::getInstance()->get('username'), $not_complete_approver)) {
                # 不是当前审批人且未通过且未审批
                $total--;
                $examine[$key]['Sort'] = 2;
                $examine[$key]['btn']  = "待审批";
    ...
    public function get_scrap_lists()
    {
        ...
        $asArr                = $asModel->DB_get_all_join('assets_scrap', 'A', $fields, $join, $where, '',
            $sort . ' ' . $order, $offset . "," . $limit);
        if (!$asArr) {
            return parent::noData();
        }
    ...
    public function get_result_lists()
    {
        ...
        //查询当前用户是否有权限进行报废处置
        $solve        = get_menu_app($this->MODULE, $this->Controller, 'result');
        $bot_span     = '<span style="color:#FFB800;">';
        $success_span = '<span style="color:green;">';
        $fail_span    = '<span style="color:red;">';
        foreach ($asArr as $k => $v) {
            if ($v['cleardate']) {
                ...
                if ($solve) {
                    if ($v['approve_status'] == 1) {
                        ...
                        $asArr[$k]['url'] = $this->returnMobileLink('查看',
                            $solve['actionurl'] . '?scrid=' . $v['scrid'], '');
                    } elseif ($v['approve_status'] == 2) {
                        $asArr[$k]['url'] = $this->returnMobileLink('不通过', 'javascript:;',
                            'layui-btn-xs layui-btn-disabled');
                        ...
                        $asArr[$k]['url'] = $this->returnMobileLink('待审核', 'javascript:;',
                            'layui-btn-xs layui-btn-disabled');
                    }
                }
            }
    ...
    public function scanQRcode_addSscrap()
    {
        ...
        $assnum   = I('get.assnum');
        //微信扫码报废进入，查询
        $exists = $this->DB_get_one('assets_info', 'assid,assnum,departid,status', [
            'assnum'      => $assnum,
            'is_delete'   => '0',
            'hospital_id' => UserInfo::getInstance()->get('current_hospitalid'),
        ]);
