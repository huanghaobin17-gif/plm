...
class PatrolPlanModel extends CommonModel
{
    ...
    public function getPatrolLists()
    {
        ...
        if ($hospital_id) {
            ...
            $where['hospital_id'] = UserInfo::getInstance()->get('current_hospitalid');
        }
        //需要的字段
        ...
        foreach ($patrol as $k => $v) {
            //查询该计划的设备
//            $patrol_assnums               = $this->DB_get_one('patrol_plans_assets',
//                'group_concat(assnum) as patrol_assnums', ['patrid' => $v['patrid']]);
//            $patrol[$k]['patrol_assnums'] = $patrol_assnums['patrol_assnums'];
            $assid_str               = $this->DB_get_one('patrol_plans_assets',
                'group_concat(assid) as assid', ['patrid' => $v['patrid']]);
            $assets_info = $this->DB_get_all('assets_info', 'assnum,assorignum,assorignum_spare', ['assid' => ['in', $assid_str['assid']]]);
            $assnums = array_column($assets_info, 'assnum');
            $assorignum = array_column($assets_info, 'assorignum');
            $assorignum_spare = array_column($assets_info, 'assorignum_spare');
            $patrol[$k]['patrol_assnums'] = implode(',', $assnums);
            $patrol[$k]['patrol_assorignum'] = implode(',', $assorignum);
            $patrol[$k]['patrol_assorignum_spare'] = implode(',', $assorignum_spare);
    ...
    public function get_plan_info($patrid)
    {
        ...
        //获取计划执行人
        $executors                   = $this->DB_get_one('patrol_plan_cycle',
            'group_concat(distinct executor) as executors,sum(abnormal_sum) as abnormal_sum',
            ['patrid' => $patrol_info['patrid'], 'patrol_level' => $patrol_info['patrol_level']]);
    ...
    public function get_plan_assets($patrol_info)
    {
        ...
        $cycle_info            = $this->DB_get_all('patrol_plan_cycle',
            'cycid,executor,assnum_tpid,plan_assnum,sign_info', $where);
        $assnum_tpid           = [];
        $assnum_name           = [];
        $assnum_cycid          = [];
        $assnum_sign_info      = [];
    ...
    public function getTasksLists()
    {
        ...
        $tasks  = $this->DB_get_all_join('patrol_plans_cycle', 'A', $fileds, $join, $where, '',
            'B.patrol_status asc,' . $sort . ' ' . $order, $offset . "," . $limit);
        if (!$tasks) {
            $result['msg']    = '暂无相关数据';
            $result['status'] = 1;
            $result['total']  = 0;
        ...
        foreach ($tasks as $key => $value) {
            ...
            $tasks[$key]['percent']          = sprintf("%.2f",
                $value['implement_sum'] / (count(json_decode($value['plan_assnum']))) * 100);
        ...
        if ($signInfo['sign_info']) {
            ...
            if (isset($repeatInfo[$exists['assnum']])) {
                ...
                $repeatInfo[$exists['assnum']] = [
                    'executor'     => UserInfo::getInstance()->get('username'),
                    'sign_in_time' => getHandleDate(time()),
                    'latitude'     => I('post.latitude'),
                    'longitude'    => I('post.longitude'),
                ];
                $res                           = $this->updateData('patrol_plan_cycle', json_encode($repeatInfo),
                    ['cycid' => $cycid]);
            }
        } else {
