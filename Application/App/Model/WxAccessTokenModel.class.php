...
class WxAccessTokenModel extends CommonModel
{
    protected $tablePrefix = 'sb_';
    protected $tableName = 'wx_access_token';

    public function bindOpenId($user)
    {
        if (UserInfo::getPlatform() == C('VUE_APP_WX')) {
            if (empty($user['openid']) && empty(UserInfo::getInstance()->get('openid'))) {
                $this->updateData('user', ['openid' => UserInfo::getInstance()->get('openid')],
                    ['userid' => $user['userid']]);
            }
            if (empty($user['qy_user_id']) && empty(UserInfo::getInstance()->get('qy_user_id'))) {
                $this->updateData('user', ['qy_user_id' => UserInfo::getInstance()->get('qy_user_id')],
                    ['userid' => $user['userid']]);
            }
        }
    }
    ...
    public function getQyAccessToken()
    {
        // access_token 应该全局存储与更新，以下代码以写入到文件中做示例
        //查找数据库token信息
        $wx          = M('wx_access_token');
        $map['item'] = 'qy_access_token';
        $data        = $wx->where($map)->find();
        if (!$data) {
            // 如果是企业号用以下URL获取access_token
            $url = "https://qyapi.weixin.qq.com/cgi-bin/gettoken?corpid=" . C('WX_WORK_APPID') . "&corpsecret=" . C('WX_WORK_SECRET');
            $res = json_decode($this->httpGet($url));
            if ($res->errcode) {
                //记录日志
                write_error('./error.html', $res->errmsg);
