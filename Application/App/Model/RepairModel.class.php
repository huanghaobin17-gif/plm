...
class RepairModel extends CommonModel
{
    ...
    public function ordersLists()
    {
        ...
        if ($hospital_id) {
            ...
            $where['A.hospital_id'] = UserInfo::getInstance()->get('current_hospitalid');
        }
        $where['A.departid'] = ['IN', UserInfo::getInstance()->get('departid')];
        ...
        $data                 = $this->DB_get_all_join('repair', 'R', $fields, $join, $where, '', $sort . ' ' . $order,
            $offset . "," . $limit);
        if (!$data) {
            $result['msg']    = '暂无相关数据';
            $result['total']  = 0;
        ...
        $acceptMenu = get_menu_app($this->MODULE, $this->Controller, 'accept');
        foreach ($data as &$one) {
            $one['department']     = $departname[$one['departid']]['department'];
            $one['applicant_time'] = getHandleMinute($one['applicant_time']);
            $one['operation']      = $this->returnMobileLink($acceptMenu['actionname'],
                $acceptMenu['actionurl'] . '?repid=' . $one['repid'], ' layui-btn-normal accept');
            if ($one['pic_url']) {
    ...
    public function accept()
    {
        ...
        $data['response']       = UserInfo::getInstance()->get('username');
        $data['response_tel']   = UserInfo::getInstance()->get('telephone');
        ...
        if ($save) {
            ...
            $push_messages[] = [
                ...
                'status_name' => '已接单',
                'time'        => date('Y-m-d H:i'),
                'username'    => UserInfo::getInstance()->get('username') . '(' . UserInfo::getInstance()->get('telephone') . ')',
            ];
    ...
    public function progress()
    {
            ...
            switch ($repairStatus) {
                case 1:
                    if (UserInfo::getInstance()->get('isSuper')) {
                        ...
                        $where['A.status']              = 1;
                        $where['is_assign']             = 1;
                        $where['assign_engineer']       = UserInfo::getInstance()->get('username');
                        $whereOR['A.status']            = 1;
                        $whereOR['_logic']              = 'or';
                        $where['_complex'][$whereIdx++] = $whereOR;
                    }
        ...
        foreach ($asArr as &$one) {
            $one['url'] = $this->returnMobileLink('查看',
                get_url() . '?repid=' . $one['repid'] . '&action=showRepairDetails', ' layui-btn layui-btn-normal');;
            $one['applicant_time'] = getHandleMinute($one['applicant_time']);
            switch ($one['status']) {
                case 1:
                    $one['status_name']       = '待接单';
    ...
    public function overhaulLists()
    {
        ...
        if (!UserInfo::getInstance()->get('isSuper')) {
            $where['response'] = ['EQ', UserInfo::getInstance()->get('username')];
        }
        $where['A.is_delete'] = '0';
        $fields               = "A.departid,A.assid,A.assets,A.assnum,A.serialnum,R.applicant,R.response_date,A.model,R.repnum,R.repid";
        $join                 = 'LEFT JOIN sb_assets_info AS A ON A.assid=R.assid';
        $total                = $this->DB_get_count_join('repair', 'R', $join, $where);
        $data                 = $this->DB_get_all_join('repair', 'R', $fields, $join, $where, '', $sort . ' ' . $order,
            $offset . "," . $limit);
        ...
        $overhaulMenu = get_menu_app($this->MODULE, $this->Controller, 'accept');
        foreach ($data as &$one) {
            $one['department']    = $departname[$one['departid']]['department'];
    ...
    public function repairApproveLists()
    {
        ...
        $hospital_id               = UserInfo::getInstance()->get('current_hospitalid');
        $where['A.departid']       = ['IN', UserInfo::getInstance()->get('departid')];
        $where['R.all_approver']   = ['LIKE', '%/' . UserInfo::getInstance()->get('username') . '/%'];
        $where['A.hospital_id']    = $hospital_id;
        $where['R.approve_status'] = ['EQ', C('REPAIR_IS_NOTCHECK')];
        ...
        $departname = [];
        include APP_PATH . "Common/cache/department.cache.php";
        $addApproveMenu = get_menu_app($this->MODULE, $this->Controller, 'addApprove');
        foreach ($data as &$one) {
            $one['department']     = $departname[$one['departid']]['department'];
            $one['applicant_time'] = getHandleDate($one['applicant_time']);

            $one['operation'] = $this->returnMobileLink($addApproveMenu['actionname'],
                $addApproveMenu['actionurl'] . '?repid=' . $one['repid'], ' layui-btn-danger');
    ...
    public function check_baoxiu()
    {
        ...
        //微信扫码进入，查询assid
        $exists = $this->DB_get_one('assets_info', 'assid,status,assnum,departid', [
            'assnum'      => $assnum,
            'hospital_id' => UserInfo::getInstance()->get('current_hospitalid'),
            'is_delete'   => '0',
        ]);
    ...
    public function scanQRCode_jianxiu()
    {
        ...
        //微信扫码检修进入，查询repid
        $exists = $this->DB_get_one('assets_info', 'assid,assnum,departid', [
            'assnum'      => $assnum,
            'is_delete'   => '0',
            'hospital_id' => UserInfo::getInstance()->get('current_hospitalid'),
        ]);
        ...
        //查询该设备维修ID
        $repinfo = $this->DB_get_one('repair', 'repid', ['assid' => $exists['assid'], 'status' => 2]);
        if (!$repinfo['repid']) {
            $result['status'] = 302;
            $msg['tips']      = '查找不到设备编码为【' . $assnum . '】的检修单信息！';
            $msg['url']       = '';
            $msg['btn']       = '';
            $result['msg']    = json_encode($msg, JSON_UNESCAPED_UNICODE);
            return $result;
...
//        \Think\Log::write('longitude==================>'.$updata['longitude']);
    ...
    public function examine()
    {
        ...
        $where['A.status']      = ['in', [C('REPAIR_MAINTENANCE_COMPLETION')]];//待验收、转单确认
        $where['B.hospital_id'] = $hospital_id;
        $where['B.departid']    = ['IN', $departids];
        switch ($sort) {
            case 'applicant_time':
        ...
        $menuData = get_menu_app($this->MODULE, 'Repair', 'checkRepair');
        //当前用户可验收科室
        foreach ($asArr as $k => $v) {
            //查询用户头像
            $uinfo                 = $this->DB_get_one('user', 'pic', ['username' => $v['appUser']]);
    ...
    public function get_repair_lists()
    {
        ...
        $where['B.departid']    = ['IN', $departids];
        if (!UserInfo::getInstance()->get('isSuper')) {
            $where['A.response'] = UserInfo::getInstance()->get('username');
        }
        switch ($sort) {
            case 'applicant_time':
                $sort = 'A.applicant_time';
        ...
        $acceptMenu = get_menu_app($this->MODULE, 'Repair', 'accept');//检修
        $startMenu  = get_menu_app($this->MODULE, 'Repair', 'startRepair');//继续维修
        //当前用户可验收科室
        ...
        foreach ($asArr as $k => $v) {
            ...
            switch ($v['status']) {
                case C('REPAIR_RECEIPT'):
                    //已接单待检修
                    if ($acceptMenu) {
                        $asArr[$k]['operation']   = $this->returnMobileLink('检修',
                            $acceptMenu['actionurl'] . '?repid=' . $v['repid'] . '&action=overhaul',
                            ' layui-btn accept');
                        $asArr[$k]['to']          = 1;
                        $asArr[$k]['status_name'] = '待检修';
                        $asArr[$k]['color']       = '#009688';
                    } else {
                        $asArr[$k]['operation']   = $this->returnMobileLink('检修', '',
                            'layui-btn layui-btn-primary nourl');
                        $asArr[$k]['to']          = 0;
                        $asArr[$k]['status_name'] = '待检修';
                        $asArr[$k]['color']       = '#969799';
                    }
                    break;
                case C('REPAIR_HAVE_OVERHAULED'):
                    //已检修/配件待出库
                    $asArr[$k]['operation']   = $this->returnMobileLink('配件出库中', 'javascript:;',
                        'layui-btn layui-btn-primary nourl');
                    $asArr[$k]['to']          = 0;
                    $asArr[$k]['status_name'] = '配件出库中';
                    $asArr[$k]['color']       = '#969799';
                    break;
                case C('REPAIR_AUDIT'):
                    //审核中
                    $asArr[$k]['operation']   = $this->returnMobileLink('审核中', 'javascript:;',
                        'layui-btn layui-btn-primary nourl');
                    $asArr[$k]['to']          = 0;
                    $asArr[$k]['status_name'] = '审核中';
                    $asArr[$k]['color']       = '#969799';
                    break;
                case C('REPAIR_MAINTENANCE'):
                    ...
                    if ($startMenu) {
                        $asArr[$k]['operation']   = $this->returnMobileLink('处理',
                            $startMenu['actionurl'] . '?repid=' . $v['repid'], ' layui-btn-normal startRepair');
                        $asArr[$k]['to']          = 1;
                        $asArr[$k]['status_name'] = '继续维修';
                        $asArr[$k]['color']       = '#009688';
                    } else {
                        $asArr[$k]['operation']   = $this->returnMobileLink('处理', '', '');
                        $asArr[$k]['to']          = 0;
        ...
        $tips             = '审批中 ' . $app_nums . ' 台；待处理 <span style="color:red;">' . $rep_nums . '</span> 台；待配件出库 ' . $part_nums . ' 台';
        $result['page']   = (int)$page;
        $result['pages']  = (int)ceil($total / C('PAGE_NUMS'));
        $result['total']  = $total;
        $result['rows']   = $asArr;
    ...
    {
        $res = $this->updateData('repair_file', ['is_delete' => 1], ['file_id' => I('post.file_id')]);
        if ($res) {
            $result['status'] = 1;
            $result['msg']    = '删除成功';
        } else {
   ...
   * */
    ...
    public function get_my_repair_orders($tasks)
    {
        ...
        $hospital_id            = UserInfo::getInstance()->get('current_hospitalid');
        $username               = UserInfo::getInstance()->get('username');
        $is_super               = UserInfo::getInstance()->get('isSuper');
        $where['B.is_delete']   = '0';
        $where['B.hospital_id'] = $hospital_id;
        $where['B.departid']    = ['in', $departids];
...
//        var_dump(M()->getLastSql());
        ...
        foreach ($data as $k => $v) {
            ...
            $data[$k]['tips_name']                         = '';
            $data[$k]['url']                               = $this->returnMobileLink('接单',
                $menu['actionurl'] . '?repid=' . $v['repid'], 'taskButtom layui-btn layui-btn-xs layui-bg-blue');
    ...
    public function get_my_repair_overhauls($tasks)
    {
        ...
        foreach ($data as $k => $v) {
            ...
            $data[$k]['department']                     = $departname[$v['departid']]['department'];
            $data[$k]['tips_name']                      = '';
            $data[$k]['url']                            = $this->returnMobileLink('检修',
                $menu['actionurl'] . '?repid=' . $v['repid'] . '&action=overhaul',
                'taskButtom layui-btn layui-btn-xs layui-bg-cyan');
    ...
    public function get_my_repair_addApprove($tasks)
    {
        ...
        if ($repair_menu) {
            ...
            $join[0]                          = "LEFT JOIN sb_assets_info AS B ON A.assid = B.assid";
            $join[1]                          = "LEFT JOIN sb_user AS C ON A.applicant = C.username";
            $data                             = $this->DB_get_all_join('repair', 'A', $fields, $join, $repair_where, '',
                'A.applicant_time DESC', '');
            foreach ($data as $k => $v) {
                $data[$k]['show_time']  = date('Y-m-d H:i', $v['applicant_time']);
                ...
                if ($v['approve_status'] == 0 && in_array(UserInfo::getInstance()->get('username'),
                        $current_approver)) {
                    $data[$k]['url']                        = $this->returnMobileLink('审批',
                        $repair_menu['actionurl'] . '?repid=' . $v['repid'],
                        'taskButtom layui-btn layui-btn-xs layui-btn-warm');
        ...
        if ($Scrapmenu) {
            ...
            $examine                    = $this->DB_get_all_join('assets_scrap', 'A', $Scrap_fields, $Scrap_join,
                $Scrap_where, '', '', '');
            if ($examine) {
                $departname = [];
                include APP_PATH . "Common/cache/department.cache.php";
                foreach ($examine as $key => &$value) {
                    ...
                    if ($value['approve_status'] == 0 && in_array(UserInfo::getInstance()->get('username'),
                            $current_approver)) {
                        $examine[$key]['url']                   = $this->returnMobileLink('审批',
                            $Scrapmenu['actionurl'] . '?scrid=' . $value['scrid'],
                            ' taskButtom layui-btn layui-btn-xs layui-btn-warm');
                        $tasks['Pub_Public_approve']['nums']    += 1;
        ...
        if ($transfer_menu) {
            ...
            foreach ($asArr as $k => $v) {
                ...
                if ($v['approve_status'] == 0 && in_array(UserInfo::getInstance()->get('username'),
                        $current_approver)) {
                    $asArr[$k]['url']                       = $this->returnMobileLink('审批',
                        $transfer_menu['actionurl'] . '?atid=' . $v['atid'], ' taskButtom layui-btn-xs layui-btn-warm');
                    $tasks['Pub_Public_approve']['nums']    += 1;
                    ...
                    if ($data) {
                        ...
                        foreach ($data as &$dataV) {
                            ...
                            if ((!$apps && $managerApproveAssid[$dataV['assid']]) or UserInfo::getInstance()->get('isSuper') == C('YES_STATUS')) {
                                //未审批,是第一个审批人
                                $dataV['url']                           = $this->returnMobileLink('审批',
                                    $departApproveBorrowMenu['actionurl'] . '?assid=' . $dataV['assid'] . '&borid=' . $dataV['borid'],
                                    ' taskButtom layui-btn-xs layui-btn-warm');
                                $tasks['Pub_Public_approve']['nums']    += 1;
    ...
    public function get_my_repair_examines($tasks)
    {
        ...
        foreach ($data as $k => $v) {
            ...
            $data[$k]['url']                           = $this->returnMobileLink('验收',
                $menu['actionurl'] . '?repid=' . $v['repid'], 'taskButtom layui-btn layui-btn-xs');
    ...
    public function get_my_transfer_examines($tasks)
    {
        ...
        $where['A.tranin_departid']     = ['in', $departids];
        ...
        foreach ($asArr as $k => $v) {
            ...
            $asArr[$k]['show_time']                         = date('Y-m-d H:i', strtotime($v['applicant_time']));
            $asArr[$k]['applicant']                         = $v['applicant_user'];
            $asArr[$k]['url']                               = $this->returnMobileLink('验收',
                $menu['actionurl'] . '?atid=' . $v['atid'], 'taskButtom layui-btn layui-btn-xs');
    ...
    {
        //查询当前用户是否有验收权限
        $menu                               = get_menu_app('Repair', 'Repair', 'getRepairLists');
        ...
        foreach ($asArr as $k => $v) {
            ...
            switch ($v['status']) {
                case C('REPAIR_AUDIT'):
                    ...
                    $asArr[$k]['url'] = $this->returnMobileLink('审核中', 'javascript:;',
                        'taskButtom layui-btn layui-btn-xs layui-btn-primary');
                    break;
                case C('REPAIR_MAINTENANCE'):
                    //继续维修
                    if ($startMenu) {
                        $asArr[$k]['url'] = $this->returnMobileLink('处理',
                            $startMenu['actionurl'] . '?repid=' . $v['repid'],
                            ' taskButtom layui-btn layui-btn-xs layui-bg-blue');
                    } else {
                        $asArr[$k]['url'] = $this->returnMobileLink('处理', '',
                            'taskButtom layui-btn layui-btn-xs layui-bg-blue');
                    }
    ...
    {
        $menu                            = get_menu_app('Assets', 'Borrow', 'borrowInCheckList');
        ...
        $borrowInCheckMenu = get_menu_app('Assets', 'Borrow', 'borrowInCheck');
        $departname        = [];
        include APP_PATH . "Common/cache/department.cache.php";
        foreach ($data as &$dataV) {
            $dataV['pic']           = $userData[$dataV['apply_userid']]['pic'];
            ...
            if ($borrowInCheckMenu) {
                $dataV['url'] = $this->returnMobileLink('借入',
    ...
    {
        $menu                             = get_menu_app('Assets', 'Borrow', 'giveBackCheckList');
        ...
        foreach ($data as &$dataV) {
            ...
            $dataV['applicant']                 = $userData[$dataV['apply_userid']]['username'];
            $dataV['pic']                       = $userData[$dataV['apply_userid']]['pic'];
            $dataV['url']                       = $this->returnMobileLink('催还',
                $menu['actionurl'] . '?action=showReminderList&borid=' . $dataV['borid'],
                ' taskButtom layui-btn layui-btn-xs layui-bg-blue');
    ...
    public function get_my_Assets_restoration($tasks)
    {
        ...
        $giveBackCheckMenu = get_menu_app('Assets', 'Borrow', 'giveBackCheck');
        foreach ($data as &$dataV) {
            $dataV['department'] = $departname[$dataV['apply_departid']]['department'];
            ...
            if ($giveBackCheckMenu) {
                $dataV['url'] = $this->returnMobileLink('归还',
    ...
    {
        $menu                        = get_menu_app('Qualities', 'Quality', 'setQualityDetail');
        ...
        if (UserInfo::getInstance()->get('isSuper')) {
            $where['is_start'] = 1;
        } elseif (UserInfo::getInstance()->get('is_supplier') == C('YES_STATUS')) {
            $where['is_start'] = 1;
            if (UserInfo::getInstance()->get('olsid') > 0) {
                $assets_f = $this->DB_get_one('assets_factory', 'GROUP_CONCAT(assid) AS assid',
                    ['ols_supid' => UserInfo::getInstance()->get('olsid')]);
                if ($assets_f['assid']) {
            ...
            $where['is_start'] = 1;
            $where['A.userid'] = ['EQ', UserInfo::getInstance()->get('userid')];
        }
        if ($hospital_id) {
            $where['B.hospital_id'] = ['EQ', $hospital_id];
        } else {
            $where['B.hospital_id'] = ['EQ', UserInfo::getInstance()->get('current_hospitalid')];
        }
        $where['B.is_delete'] = '0';
        ...
        foreach ($data as $k => $v) {
            ...
            $data[$k]['department']        = $departname[$v['departid']]['department'];
            $data[$k]['tips_name']         = '';
            $data[$k]['url']               = $this->returnMobileLink('检测', $menu['actionurl'] . '?qsid=' . $v['qsid'],
                'taskButtom layui-btn layui-btn-xs  layui-bg-blue');
    ...
    {
        //查询当前用户是否有出库权限
        $menu                                              = get_menu_app('Repair', 'RepairParts', 'partsOutWare');
        ...
        $whereP['status']      = 0;
        $reps                  = $this->DB_get_one('parts_outware_record',
            'group_concat(repid) as repids,group_concat(outwareid) as outwareid', $whereP);
        if ($reps['repids']) {
            ...
            foreach ($data as $k => $v) {
                ...
                $data[$k]['department']                              = $departname[$v['departid']]['department'];
                $data[$k]['tips_name']                               = '';
                $data[$k]['url']                                     = $this->returnMobileLink('出库',
                    $menu['actionurl'] . '?repid=' . $v['repid'], 'taskButtom layui-btn layui-btn-xs layui-bg-orange');
    ...
    public function get_repair_info($repid)
    {
        ...
        $join       = "LEFT JOIN sb_assets_info AS B ON A.assid = B.assid";
        $repinfo    = $this->DB_get_one_join('repair', 'A', 'A.*,B.assets,B.assnum,B.departid,B.catid,B.model,B.brand',
            $join, ['A.repid' => $repid, 'B.is_delete' => '0']);
        $departname = [];
        include APP_PATH . "Common/cache/department.cache.php";
    ...
    public function get_parts_stock($parts)
    {
        ...
        foreach ($parts as $k => $v) {
            ...
            $where['repid']                = [['eq', 0], ['eq', $repid], 'or'];//未出库
            $where['leader']               = [['eq', ''], ['eq', $username], 'or'];//领用人
    ...
    public function get_repair_examine()
    {
        ...
        $where['A.all_approver']   = ['LIKE', '%/' . UserInfo::getInstance()->get('username') . '/%'];
        $where['B.hospital_id']    = $hospital_id;
        $where['B.departid']       = ['IN', $departids];
        switch ($sort) {
            case 'applicant_time':
        ...
        $departname = [];
        include APP_PATH . "Common/cache/department.cache.php";
        $canApproval    = get_menu_app('Repair', 'Repair', 'addApprove');
        $addApproveMenu = get_menu_app($this->MODULE, $this->Controller, 'addApprove');
        foreach ($data as &$one) {
            $one['department']     = $departname[$one['departid']]['department'];
            ...
            if ($one['current_approver']) {
                ...
                if ($current_approver_arr[UserInfo::getInstance()->get('username')]) {
                    $one['Sort'] = 1;
                    if ($canApproval) {
                        $one['btn'] = "审批";
                    } else {
                    ...
                    if (!in_array(UserInfo::getInstance()->get('username'),
                            $complete) && in_array(UserInfo::getInstance()->get('username'), $notcomplete)) {
                        //完全未审
                        $one['btn'] = "待审批";
                    } elseif (in_array(UserInfo::getInstance()->get('username'),
                            $complete) && in_array(UserInfo::getInstance()->get('username'), $notcomplete)) {
                        //有已审，有未审
                        $one['btn'] = "待审批";
                    } elseif (in_array(UserInfo::getInstance()->get('username'),
