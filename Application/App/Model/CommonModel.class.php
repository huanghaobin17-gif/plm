...
class CommonModel extends Model
{
    ...
    */
    ...
    public function addLog($tableName, $sql, $text, $actionid = '', $actionName = '')
    {
        if (C('IS_OPEN_LOG')) {
            //当前用户id
            $addData['username'] = UserInfo::getInstance()->get('username');
            if ($tableName) {
                //如果存在表就存表
...
//        if (UserInfo::getInstance()->get('isSuper') && C('IS_OPEN_BRANCH')) {
//            //查询当前用户所在的医院代码
//            $existsid = explode(',', UserInfo::getInstance()->get('manager_hospitalid'));
//        } else {
//            $existsid = explode(',', UserInfo::getInstance()->get('job_hospitalid'));
//        }
        $user = $this->DB_get_all('user', 'userid,username',
            ['status' => 1, 'is_delete' => 0, 'job_hospitalid' => UserInfo::getInstance()->get('current_hospitalid')],
            '', 'userid asc', '');
        return $user;
    }
    ...
    public function updateAssetsStatus($assid, $status, $remark)
    {
        $this->insertData('assets_state_change', [
            ...
            'new_status'  => $status,
            'remark'      => $remark,
            'change_user' => UserInfo::getInstance()->get('username'),
    ...
    public function updateAllAssetsStatus($update_assid_arr, $status, $remark)
    {
        ...
        foreach ($update_assid_arr as $key => $value) {
            ...
            $addAllChange[$key]['change_user'] = UserInfo::getInstance()->get('username');
    ...
    public function checkSmsIsOpen($model)
    {
        ...
        if (!$settingData) {
            ...
            if (!$Data) {
                $Data = M('sms_basesetting')->where(['hospital_id' => UserInfo::getInstance()->get('current_hospitalid')])->cache('sms_basesetting',
                    3600)->select();
            }
            if (!$Data) {
                return false;
    ...
    public function checkDepartNum($departnum)
    {
        if (C('DEPART_PREFIX_NUM') == '') {
            return ['status' => 1, 'msg' => '系统未设置部门编码规则', 'num' => $departnum];
        }
        ...
        if ($data['process_node_level'] == $lastProcess || $data['is_adopt'] == C('STATUS_APPROE_FAIL')) {
            ...
            if ($data['is_adopt'] == C('STATUS_APPROE_SUCCESS')) {
                ...
                if ($approveType == C('REPAIR_APPROVE')) {
                    //修改维修表中status为维修中
                    $this->updateData($tableName, ['status' => C('REPAIR_MAINTENANCE')], [$idName => $arr[$idName]]);
                    //最后一次审批通过以后生成一条维修合同
            ...
            if ($data['is_adopt'] == C('STATUS_APPROE_FAIL')) {
                ...
                if ($approveType == C('OUTSIDE_APPROVE')) {
                    //本次为最后一道审批，且为不同意外调的
                    $this->updateData('assets_outside', ['status' => C('OUTSIDE_STATUS_FAIL')],
                        ['outid' => $arr['outid']]);
                    ...
                    //本次为最后一道审批，且为不同意报废的
                    $this->updateData('assets_scrap', ['approve_status' => C('STATUS_APPROE_FAIL')],
                        ['scrid' => $arr['scrid']]);
                    ...
                    //不同意，更新设备最新状态为在用
                    $this->updateData('assets_info', ['status' => C('ASSETS_STATUS_USE')], ['assid' => $arr['assid']]);
                }
                //记录设备变更信息
    ...
    public function formatPhone($userData)
    {
        ...
        foreach ($userData as &$one) {
            if ($userData['username'] != UserInfo::getInstance()->get('username')) {
                $phone .= ',' . $one['telephone'];
            }
        }
        $phone = ltrim($phone, ",");
    ...
    public function scrapResultSms($data, $status)
    {
        ...
        if ($settingData) {
            ...
            $scrapInfo            = $this->DB_get_one('assets_scrap', 'scrapnum,assid,apply_user',
                ['scrid' => $data['scrapid']]);
            $asInfo               = $this->DB_get_one('assets_info', 'assnum,assets',
                ['assid' => $scrapInfo['assid'], 'is_delete' => '0']);
...
//        }
    ...
    //发送微信消息给特定用户
    public function sendMsgToOnUserByWechat($openid, $templateId, $url, $data)
    {
        if ($openid) {
    ...
    public function getToUser($uid, $departid, $moduleName, $controllerName, $actionName)
    {
        ...
        $role_where['B.hospital_id'] = UserInfo::getInstance()->get('current_hospitalid');
            ...
            foreach ($apps as $kp => $vp) {
                if ($vp[$app_id] == $data[$data_id]) {
                    ...
                    foreach ($all_approver as $uk => $username) {
                        if ($is_adopt[$uk]) {
                            if ($is_adopt[$uk] == 1) {
                                $html         .= '<li><div class="timeLineBox"><i class="layui-icon layui-timeline-axis ' . $pass_color . '">' . $dui_icon . '</i><div class="timeLine ' . $pass_color . 'Bg"></div><span class="timeLineTitle ' . $pass_color . '">' . $username . '</span><span class="timeLineDate ' . $pass_color . '">' . date('Y-m-d',
                                        $approve_time[$uk]) . '</span></div></li>';
                                $app_user_num = $app_user_num + 1;
                            } else {
                                $html .= '<li><div class="timeLineBox"><i class="layui-icon layui-timeline-axis ' . $no_pass_color . '">' . $cuo_icon . '</i><div class="timeLine ' . $no_pass_color . 'Bg"></div><span class="timeLineTitle ' . $no_pass_color . '">' . $username . '</span><span class="timeLineDate ' . $no_pass_color . '">' . date('Y-m-d',
                                        $approve_time[$uk]) . '</span></div></li>';
                            }
                        } else {
                            $html .= '<li><div class="timeLineBox"><i class="layui-icon layui-timeline-axis ' . $gray_color . '">' . $yuan_icon . '</i><div class="timeLine ' . $gray_color . 'Bg"></div><span class="timeLineTitle ' . $gray_color . '">' . $username . '</span><span class="timeLineDate ' . $gray_color . '">-</span></div></li>';
