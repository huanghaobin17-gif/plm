...
class RepairPartsModel extends CommonModel
{
    ...
    public function get_parts_orders()
    {
        ...
        //查询配件表
        $rep_parts = $this->DB_get_all('repair_parts', 'repid,sum(part_num) as parts_num', ['status' => 0], 'repid');
        if (!$rep_parts) {
            $result['msg']    = '暂无相关数据';
            $result['status'] = 1;
            $result['total']  = 0;
        ...
        $where['A.repid']       = ['in', $repidsarr];//待验收、转单确认
        ...
        $menuData = get_menu_app($this->MODULE, $this->Controller, 'partsOutWare');
        //当前用户可验收科室
        foreach ($asArr as $k => $v) {
            //查询用户头像
            $uinfo                       = $this->DB_get_one('user', 'pic', ['username' => $v['response']]);
    ...
    public function get_parts_stock()
    {
        ...
        if ($search) {
            $map['parts']         = ['like', '%' . $search . '%'];
            $map['parts_model']   = ['like', '%' . $search . '%'];
            $map['supplier_name'] = ['like', '%' . $search . '%'];
            $map['_logic']        = 'or';
            $where['_complex']    = $map;
        }
        ...
        if ($search) {
            $joinwhere['A.parts']      = ['IN', $parts_str];
            $joinwhere['A.part_model'] = ['IN', $model_str];
        }
        $joinwhere['B.status'] = ['IN', '3,7'];
        ...
        $partsOutWare          = get_menu_app('Repair', 'RepairParts', 'partsOutWare');
    ...
    {
        return $this->DB_get_one('parts_inware_record', '*', ['repid' => $repid]);
    }
}
