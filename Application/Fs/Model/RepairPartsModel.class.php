...
class RepairPartsModel extends CommonModel
{
    ...
    public function get_parts_stock()
    {
            ...
            foreach ($data as $kk => $vv) {
                if ($v['parts'] == $vv['parts'] && $v['part_model'] == $vv['parts_model'] && $v['adduser'] == $vv['leader'] && $v['repid'] == $vv['repid']) {
                    ...
                    if ($vv['stock_nums'] < $v['part_num']) {
                        $rurl = C('FS_NAME') . '/RepairParts/partsInWare.html?action=partsInWareApply&repid=' . $vv['repid'] . '&sname=' . $vv['supplier_name'] . '&sid=' . $vv['supplier_id'] . '&stock=' . $vv['stock_nums'] . '&parts=' . $vv['parts'] . '&model=' . $vv['parts_model'] . '&price=' . $vv['price'];
                        $data[$kk]['cus_sort'] = 2;
                        ...
                        $rurl = C('FS_NAME') . '/RepairParts/partsInWare.html?action=partsInWareApply&repid=' . $v['repid'];
                        $data[$kk]['cus_sort'] = 1;
                        $data[$kk]['repnum'] = '（' . $v['repnum'] . '）';
                        $data[$kk]['need_nums'] = '（需求量：' . $v['part_num'] . $vv["unit"] . '）';
                        $data[$kk]['type'] = 2;
        ...
        foreach ($repairparts as $k => $v) {
            $rurl = C('FS_NAME') . '/RepairParts/partsInWare.html?action=partsInWareApply&repid=' . $v['repid'] . '&sname=' . $supplier_name_arr[$v['parts']][$v['part_model']] . '&sid=' . $supplier_id_arr[$v['parts']][$v['part_model']] . '&stock=0&parts=' . $v['parts'] . '&model=' . $v['part_model'] . '&price=' . $price_arr[$v['parts']][$v['part_model']];
            $v['cus_sort'] = 2;
            $v['repnum'] = '（' . $v['repnum'] . '）';
