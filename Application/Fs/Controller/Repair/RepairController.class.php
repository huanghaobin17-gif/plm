...
namespace Fs\Controller\Repair;
...
//        $feishu_fields[0]['is_short'] = false;//是否并排布局
//        $feishu_fields[0]['text']['content'] = '**报修人：**'.$_SESSION['username'];
//        $feishu_fields[0]['text']['tag'] = 'lark_md';
//        $feishu_fields[1]['is_short'] = false;
//        $feishu_fields[1]['text']['content'] = '**维修单号：**wx20190717_0001';
//        $feishu_fields[1]['text']['tag'] = 'lark_md';
//        $feishu_fields[2]['is_short'] = false;
//        $feishu_fields[2]['text']['content'] = '**设备名称：**全自动血液分析仪';
//        $feishu_fields[2]['text']['tag'] = 'lark_md';
//        $feishu_fields[3]['is_short'] = false;
//        $feishu_fields[3]['text']['content'] = '**设备编码：**684001303355';
//        $feishu_fields[3]['text']['tag'] = 'lark_md';
//        $feishu_fields[4]['is_short'] = false;
//        $feishu_fields[4]['text']['content'] = '**设备型号：**XP-100';
//        $feishu_fields[4]['text']['tag'] = 'lark_md';
//        $feishu_fields[5]['is_short'] = false;
//        $feishu_fields[5]['text']['content'] = '**使用科室：**婚前孕前体检中心';
//        $feishu_fields[5]['text']['tag'] = 'lark_md';
//
//        $feishu_actions[0]['tag'] = 'button';
//        $feishu_actions[0]['text']['content'] = '查看详情';
//        $feishu_actions[0]['text']['tag'] = 'plain_text';
//        $feishu_actions[0]['type'] = 'primary';
//        $feishu_actions[0]['url'] = C('APP_NAME') .C('FS_FOLDER_NAME').'/#' . C('FS_NAME') . '/Repair/accept?repid=538';;//指定跳转地址
//
//
...
//
//        //$toUser = $this->getToUser($_SESSION['userid'], $info['departid'], $moduleName, $controllerName, $actionName);
//        //foreach ($toUser as $k => $v) {
//        $ar = new \Admin\Model\RepairModel();
//        $ar->send_feishu_card_msg('ou_29217b5ea4ec7db5ead19f95660f3acd',$card_data);
//        //}
//exit;
    ...
    public function addRepair()
    {
        if (IS_POST) {
            ...
            $offlineSuppliers = $OfflineSuppliersModel->DB_get_one('assets_factory', 'ols_facid,ols_supid,ols_repid', array('assid' => $asArr['assid']));
            $factoryInfo = [];
            $supplierInfo = [];
            $repairInfo = [];
                ...
                case 'edit_engineer':
                    ...
                    if (!$repInfo) {
                        ...
                        if ($res) {
                            ...
                            $fd['is_short'] = false;//是否并排布局
                            $fd['text']['tag'] = 'lark_md';
                            $fd['text']['content'] = '**设备编号：**'.$repInfo['assnum'];
                            $feishu_fields[] = $fd;
                            ...
                            $fd['is_short'] = false;//是否并排布局
                            $fd['text']['tag'] = 'lark_md';
                            $fd['text']['content'] = '**转单人：**'.session('username');
                            $feishu_fields[] = $fd;

                            $fd['is_short'] = false;//是否并排布局
                            $fd['text']['tag'] = 'lark_md';
                            $fd['text']['content'] = '**故障描述：**'.$repInfo['breakdown'];
                            $feishu_fields[] = $fd;

                            $fd['is_short'] = false;//是否并排布局
                            $fd['text']['tag'] = 'lark_md';
                            $fd['text']['content'] = '请及时接单维修';
                            $feishu_fields[] = $fd;

                            //按钮区域
                            $act['tag'] = 'button';
                            $act['type'] = 'primary';
                            $act['url'] = C('APP_NAME') . C('FS_FOLDER_NAME') . '/#' . C('FS_NAME') . '/Repair/accept?action=overhaul&repid=' . $repid;
                            $act['text']['tag'] = 'plain_text';
                            $act['text']['content'] = '查看详情并接单';
                            $feishu_actions[] = $act;
                            ...
                            $card_data['elements'][2]['layout'] = 'bisected';
                            $card_data['elements'][2]['tag'] = 'action';
                            $card_data['header']['template'] = 'yellow';
                            $card_data['header']['title']['content'] = '设备维修转单提醒';
                            $card_data['header']['title']['tag'] = 'plain_text';

                            $RepairModel->send_feishu_card_msg($user['openid'],$card_data);
                            //==========================================飞书 END==========================================
                            $result['status'] = 1;
                            $result['msg'] = '转单成功';
                        } else {
    ...
    public function checkRepair()
    {
        if (IS_POST) {
            ...
            if ($repArr['status'] == C('REPAIR_ALREADY_ACCEPTED')) {
                //已验收，跳转到详情页
                $result['status'] = 302;
                $result['redirectUrl'] = C('FS_NAME').'/Repair/showRepairDetails?repid=' . $repid;
                $this->ajaxReturn($result, 'json');
            }
            //设备信息
            $asArr = $RepairModel->getAssetsBasic($repArr['assid']);
            ...
            if ($repid) {
                ...
                if ($repArr['all_approver']) {
                    ...
                    foreach ($all_approver as $k => $v) {
                        $upic = $RepairModel->DB_get_one('user', 'username,pic', array('username' => $v));
