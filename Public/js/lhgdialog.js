			...
			'<td class="ui_c">' +
				...
				'<table class="ui_dialog">' +
					'<tbody>' +
						...
						'</tr>' +
						'<tr>' +
							'<td class="ui_icon" valign=top></td>' +
...
lhgdialog.fn = lhgdialog.prototype =
{
	...
	lock: function()
	{
		...
		if( !mask )
		{
			frm = '<iframe src="javascript:\'\'" style="width:100%;height:100%;position:absolute;' +
			    'top:0;left:0;z-index:-1;filter:alpha(opacity=0)"></iframe>';//浮动背景层
	...
	close: function()
	{
		...
		if( that.iframe )
		{
			...
			_$html.removeClass('ui_lock_scroll');//重新找回滚动条
	...
	max: function()
	{
		...
		if( !that._maxState )
		{
			...
			DOM.res[0].style.display = 'none';//****************tcdahe改
	...
	max_div: function()
	{
		...
		if( !that._maxState )
		{
			...
			wrapStyle.top = top + 'px';
			//wrapStyle.left = left + 'px';	
			wrapStyle.left = '18%';//***********************************tcdahe改
			maxSize = that._maxSize_div();
			that.size( maxSize.w, maxSize.h )._setAbsolute();
	...
	_iframe: function( url )
	{
		...
		var load = that._fmLoad = function()
		{
			...
			var autoheight,pmheight,qdheight,autowidth,pmwidth,qdwidth          
			autoheight=$(this).contents().find("body").height()+10;
			if (autoheight==0)
			{autoheight=config.height;}
		//	alert(autoheight)
			autowidth=$(this).contents().find("body").width();
			pmheight=document.documentElement.offsetHeight;
			pmwidth=document.documentElement.offsetWidth;

            if (autoheight>pmheight)//判断读取的高度是否大于屏幕高度
            {qdheight=pmheight-50;_$html.addClass('ui_lock_scroll'); document.onmousewheel = function() {return false;}}//高于屏幕高度以屏幕高度为主
			else{qdheight=autoheight;}//低于屏幕高度则全部显示
            if (autowidth>pmwidth)//判断读取的高度
            {qdwidth=pmwidth;}//高于屏幕高度以屏幕高度为主
			else{qdwidth=autowidth;}//低于屏幕高度则全部显示
			...
			loading.style.cssText = 'display:none;position:absolute;background:#FFF;opacity:0;' + 
			    'filter:alpha(opacity=0);z-index:1;width:' + qdwidth + ';height:' + qdheight + ';';
	...
	_maxSize: function()
	{
		...
		return { w: 950, h: maxHeight };//***********************************tcdahe改
	},
	_maxSize_div: function()
	{
	    var that = this, DOM = that.DOM,
		    wrap = DOM.wrap[0],
			main = DOM.main[0],
		...
		maxHeight = _$top.height() - wrap.offsetHeight + main.offsetHeight;
		//return { w: maxWidth, h: maxHeight };
		return { w: 1250, h: maxHeight };//***********************************tcdahe改
	},
	_maxSize_big: function()
	{
	    var that = this, DOM = that.DOM,
		    wrap = DOM.wrap[0],
			main = DOM.main[0],
...
$(function(){
	...
	(function(dragEvent){
		...
		+ 'cursor:move;filter:alpha(opacity=0.7);opacity:0;background:#111;pointer-events:none;';
